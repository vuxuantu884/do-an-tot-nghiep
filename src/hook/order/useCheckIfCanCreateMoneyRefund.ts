...
function useCheckIfCanCreateMoneyRefund(isReceivedReturnProducts: boolean, orderDetail: OrderResponse | null) {
  const [allowCreateMoneyReturn] = useAuthorization({
    acceptPermissions: [ODERS_PERMISSIONS.CREATE_MONEY_REFUND],
    not: false,
  });
  
  const canCreateMoneyRefund = (isOrderFromPOS(orderDetail) || allowCreateMoneyReturn) && isReceivedReturnProducts
