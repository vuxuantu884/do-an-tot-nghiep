...
function useFetchConversionRate() {
  ...
  useEffect(() => {
    const fetchTotalSaleComplete = async () => {
      ...
      const data = await getDataOneQueryDashboard(
        dispatch,
        showMyData,
        deparmentIdList,
        BUSINESS_RESULT_CONVERSION_RATE_QUERY,
      );
      ...
      setDataSrcBusinessResultCard((prevState: Map<string, BusinessResultCartItem>) => {
        ...
        newState.set(BUSINESS_RESULT_CART_NAME.conversionRate, {
          value: conversionRateMilstone / successMilestone,
          monthlyAccumulated: accRate,
        });
        return newState;
      });
    };
