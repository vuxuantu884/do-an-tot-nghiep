...
} from "config/dashboard/business-result-config";
...
function useFetchChartBusinessResult() {
  ...
  const { setDataSrcChartBusinessResult, setTotalSalesToday, deparmentIdList, showMyData } =
  ...
  useEffect(() => {
    ...
    const fetchChartData = async () => {
      ...
      // tính luỹ kế 3 tháng
      const accumulatedIncome = data[0].result.data.reduce(
        (accumulator: Array<AccumulateData>, currentValue: ArrayAny) => {
