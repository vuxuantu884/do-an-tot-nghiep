...
function useFetchPreTotalSaleCanceled() {
  const dispatch = useDispatch();
  const { setDataSrcBusinessResultCard, deparmentIdList, showMyData } =
    useContext(DashboardContext);
  const [isFetchingPreTotalSaleCanceled, setIsFetchingPreTotalSaleCanceled] =
    useState<boolean>(false);

  useEffect(() => {
    const fetchTotalSaleComplete = async () => {
      setIsFetchingPreTotalSaleCanceled(true);
      ...
      setIsFetchingPreTotalSaleCanceled(false);
      if (!response) {
      ...
      setDataSrcBusinessResultCard((prevState: Map<string, BusinessResultCartItem>) => {
        ...
        newState.set(BUSINESS_RESULT_CART_NAME.cancelledPreTotalSales, {
  ...
  return { isFetchingPreTotalSaleCanceled };
}

export default useFetchPreTotalSaleCanceled;
