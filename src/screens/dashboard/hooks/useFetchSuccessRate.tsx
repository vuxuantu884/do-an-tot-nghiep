...
function useFetchSuccessRate() {
  ...
  useEffect(() => {
    const fetchingSuccessRate = async () => {
      ...
      const dataDay = await getDataOneQueryDashboard(
        dispatch,
        showMyData,
        deparmentIdList,
        BUSINESS_RESULT_SUCCESS_RATE_DAY_QUERY,
      );
      const dataMonth = await getDataOneQueryDashboard(
        dispatch,
        showMyData,
        deparmentIdList,
        BUSINESS_RESULT_SUCCESS_RATE_MONTH_QUERY,
      ...
      setDataSrcBusinessResultCard((prevState: Map<string, BusinessResultCartItem>) => {
        ...
        newState.set(BUSINESS_RESULT_CART_NAME.successRate, {
          value: successRateDay.summary[0]
            ? +((100 * successRateDay.summary[1]) / successRateDay.summary[0]).toFixed(0)
            : 0,
          monthlyAccumulated: successRateMonth.summary[0]
            ? +((100 * successRateMonth.summary[1]) / successRateMonth.summary[0]).toFixed(0)
            : 0,
        });
        return newState;
      });
