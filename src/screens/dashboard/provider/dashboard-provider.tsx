...
type DashboardProviderValue = {
  ...
  setDataSrcBusinessResultCard: React.Dispatch<
    React.SetStateAction<Map<string, BusinessResultCartItem>>
  >;
...
export const DashboardContext = React.createContext<DashboardProviderValue>(
  {} as DashboardProviderValue,
);
...
function DashboardPrivider(props: { children: React.ReactNode }) {
  ...
  const [dataSrcBusinessResultCard, setDataSrcBusinessResultCard] = React.useState<
    Map<string, BusinessResultCartItem>
  >(new Map());
  const [dataSrcChartBusinessResult, setDataSrcChartBusinessResult] = React.useState<
    Array<DayTotalSale>
  >([]);
  ...
  const [showMyData, setShowMyData] = React.useState<DashboardShowMyData>({
    isSeeMyData: false,
  });
  return (
