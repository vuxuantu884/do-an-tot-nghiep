...
const BankAccountScreen: React.FC = () => {
  ...
  const defaultColumns: Array<ICustomTableColumType<BankAccountResponseResult>> = [
    {
      ...
      render: (value, row, index) => {
        return (
          <span>
            {(params?.page ? params.page - 1 : 0) * (params?.limit ? params.limit : 0) + index + 1}
          </span>
        );
      },
    },
      ...
      render: (value, row: BankAccountResponse, index) => {
        return (
          <React.Fragment>
            ...
            <div style={{ fontSize: "0.86em" }}>{row.bank_name}</div>
            <div style={{ fontSize: "0.86em" }}>{row.account_holder}</div>
          </React.Fragment>
        );
      },
    },
          ...
          return (
            <React.Fragment>
              {row?.stores?.map((item, index) => {
                return (
                  <span>
                    {item?.store_name} {index + 1 < row.stores.length ? ", " : ""}
                  </span>
                );
              })}
            </React.Fragment>
          ...
          return (
            <React.Fragment>
              <span>
                ...
                <Button type="link" style={{ padding: 0 }} onClick={() => showAllInOneRow(index)}>
                  ...xem thêm
                </Button>
              </span>
            </React.Fragment>
      ...
      render: (value, row, index) => {
        ...
        return (
          <div
            style={
              row.default
                ? { textAlign: "center", fontSize: "20px", color: "#2A2A86" }
  ...
  const handleSearchResult = useCallback(() => {
    ...
    dispatch(
      getBankAccountAction(params, (data: PageResponse<BankAccountResponse>) => {
        // setAbc(data.items);
