...
const SmsAccountSetting: React.FC<any> = (props: any) => {
  ...
  const handleSmsAccountData = useCallback(
    (data: any) => {
      if (data) {
        form.setFieldsValue({
  ...
  return (
    <StyledSmsSetting>
      <Card title={"CÀI ĐẶT TÀI KHOẢN GỬI TIN"} className="sms-settings">
        <Form form={form} name="sms-settings" onFinish={handleUpdateSmsConfig} layout="vertical">
          ...
          <div className="row-item">
            ...
            <Form.Item name="is_unicode" label={<b>Gửi tin có dấu:</b>} className="right-item">
              <Switch
                checked={isUnicode}
                onChange={(checked) => {
