...
import { BasePrinterModel, FormPrinterModel } from "model/response/printer.response";
...
function SinglePrinter() {
  ...
  const [singlePrinterContent, setSinglePrinterContent] = useState<BasePrinterModel>({
  ...
  const [actionSinglePrinter, setActionSinglePrinter] = useState<FormPrinterModel>(viewAction);
  ...
  return (
    <StyledComponent>
      <ContentContainer
        title={queryParams.action === editAction ? "Tùy chỉnh mẫu in" : "Chi tiết mẫu in"}
        breadcrumb={queryParams.action === editAction ? breadCrumbEdit : breadCrumbViewDetail}
      >
        <FormPrinter
