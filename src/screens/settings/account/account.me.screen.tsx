...
const AccountMeScreen: React.FC = () => {
  ...
  const updateMeContact = useCallback(
    (data: MeRequest) => {
      ...
      if (user?.id) {
        dispatch(
          updateMeAction(accountUpdate, (res) => {
            if (res) {
  ...
  return (
    ...
    >
      <AccountMeStyle>
            ...
            <Card className="general">
              ...
              <div className="general-info">
                ...
                <RowDetail
                  ...
                  value={
                    user?.gender
                      ? listGender?.find((item) => item.value === user?.gender)?.name ?? ""
                      : "---"
                  }
                />
                <RowDetail
                  ...
                  value={
                    user?.birthday
                      ? ConvertUtcToLocalDate(user.birthday, DATE_FORMAT.DDMMYYY)
                      : "---"
                  }
                />
                <Row>
          ...
          <Col span={16}>
            <Card
              ...
              extra={
                ...
                >
                  <EditOutlined /> Đổi thông tin liên hệ
                </Button>
              }
            >
            ...
            >
              <RowDetail
                title="Phòng ban"
                value={
                  objOrther?.departments && objOrther.departments?.length > 0
                    ? objOrther.departments
                    : "---"
                }
              />
              <RowDetail
                title="Vị trí"
                value={objOrther?.jobs && objOrther.jobs?.length > 0 ? objOrther?.jobs : "---"}
              />
              <RowDetail title="Nhóm quyền" value={objOrther?.permissions?.role_name ?? "---"} />
              <RowDetail
                title="Cửa hàng"
                value={
                  objOrther?.stores && objOrther.stores?.length > 0 ? (
                    <TextShowMore>{objOrther.stores}</TextShowMore>
                  ) : (
                    "---"
                  )
                }
        ...
        <BottomBarContainer back="Quay lại trang chủ" backAction={backAction} />
      </AccountMeStyle>
      <CustomModal
        updateText="Lưu lại"
