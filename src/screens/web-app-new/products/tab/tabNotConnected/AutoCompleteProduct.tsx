...
const AutoCompleteProduct = (props: AutoCompleteProductProps) => {
    ...
    const saveConnectYodyProduct = () => {
        ...
        setIsSaving(true);
        dispatch(
            putConnectWebAppProductAction(request, callbackSaveConnectProduct)
        );
    };
    ...
    const handleSelectVariant = (id: any) => {
        const item = variantList && variantList.items && variantList.items.find((item) => item.id === id);
        if(item){
            const variant = {
                core_variant: item.name,
                core_sku: item.sku,
                variant_prices: item.variant_prices,
                core_price: item.variant_prices &&  item.variant_prices[0] && item.variant_prices[0].retail_price,
                id: lineItem.id,
