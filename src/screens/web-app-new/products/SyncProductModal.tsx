import { Checkbox, Form, Modal, Radio, Select, Space } from "antd";
...
const SyncProductModal = (props: SyncProductModalProps) => {
  ...
  useEffect(() => {
    dispatch(getWebAppShopList({}, updateEcommerceShopList));
  }, [dispatch, updateEcommerceShopList]);

  const handleCheckedChange = (shop: any, e: any) => {
    if (e.target.checked) {
      ...
      <StyledComponentSyncStock>
        <Radio.Group onChange={onChangeSyncOption} value={syncStockType}>
          <Space direction="vertical">
            <Radio value={"selected"} disabled={isDisabled} onChange={handleSyncShopListSelected}>
