...
const WebAppProducts = () => {
          ...
          if (responses.code === HttpStatus.SUCCESS) {
            if (processData.finish) {
              ...
              const percent = Math.floor(
                (responses.data.total_success / responses.data.total) * 100,
              );
  ...
  useEffect(() => {
    if (processId) {
      if (
        statusExport === ExportFileStatus.Exporting &&
        processType === ProcessTypeContants.Export
      ) {
        checkExportFile();
      }
      if (
  ...
  const handleDownloadProduct = (params: any) => {
    ...
    dispatch(
      downloadWebAppProductAction(params, (data: any) => {
        if (data) {
