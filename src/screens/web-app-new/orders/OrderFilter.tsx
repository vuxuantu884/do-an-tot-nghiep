    ...
    return (
      <Menu>
        {actionList?.map((item: any) => (
          <Menu.Item disabled={item.disabled} key={item.id} onClick={item.onClick} icon={item.icon}>
            {item.name}
          </Menu.Item>
        ))}
      </Menu>
  ...
  useEffect(() => {
    ...
    if (params.issued_on_min || params.issued_on_max) {
      let text =
        (params.issued_on_min
          ? moment(params.issued_on_min, "DD-MM-YYYY").format("DD-MM-YYYY")
          : "??") +
        " ~ " +
        (params.issued_on_max
          ? moment(params.issued_on_max, "DD-MM-YYYY").format("DD-MM-YYYY")
          : "??");
      filters.push({
        key: "issued",
        name: "Ngày tạo đơn",
    ...
    if (params.completed_on_min || params.completed_on_max) {
      let textOrderCompleteDate =
        (params.completed_on_min
          ? moment(params.completed_on_min, "DD-MM-YYYY").format("DD-MM-YYYY")
          : "??") +
        " ~ " +
        (params.completed_on_max
          ? moment(params.completed_on_max, "DD-MM-YYYY").format("DD-MM-YYYY")
          : "??");
      filters.push({
        key: "completed",
    ...
    if (params.cancelled_on_min || params.cancelled_on_max) {
      let textOrderCancelDate =
        (params.cancelled_on_min
          ? moment(params.cancelled_on_min, "DD-MM-YYYY").format("DD-MM-YYYY")
          : "??") +
        " ~ " +
        (params.cancelled_on_max
          ? moment(params.cancelled_on_max, "DD-MM-YYYY").format("DD-MM-YYYY")
          : "??");
      filters.push({
        key: "cancelled",
        name: "Ngày huỷ đơn",
    ...
    if (params.sub_status_code && params.sub_status_code.length) {
      ...
      params.sub_status_code.forEach((statusCode: any) => {
        const findStatus = subStatusList?.find(
          (item) => item.code.toString() === statusCode.toString(),
        );
    ...
    if (params.assignee_codes && params.assignee_codes.length > 0) {
      ...
      params.assignee_codes.forEach((i) => {
        ...
        textAccount = findAccount
          ? textAccount + findAccount.code + " - " + findAccount.full_name + "; "
          : textAccount;
      });
      filters.push({
        key: "assignee_codes",
    ...
    if (params.price_min || params.price_max) {
      let textPrice =
        (params.price_min ? `${params.price_min} đ`.replace(/\B(?=(\d{3})+(?!\d))/g, ",") : " 0 ") +
        " ~ " +
        (params.price_max ? `${params.price_max} đ`.replace(/\B(?=(\d{3})+(?!\d))/g, ",") : " ?? ");
      filters.push({
        key: "price",
        name: "Tổng tiền",
        value: textPrice,
  ...
  const handleRemoveTag = useCallback(
    (e: any, tag: any) => {
      ...
      if (tag.key === "issued") {
        newParams = {
          ...newParams,
          ...{ issued_on_min: null, issued_on_max: null },
        };
        setIssuedClick("");
      } else if (tag.key === "completed") {
        newParams = {
          ...newParams,
          ...{ completed_on_min: null, completed_on_max: null },
        };
        setCompletedClick("");
      } else if (tag.key === "cancelled") {
        newParams = {
          ...newParams,
          ...{ cancelled_on_min: null, cancelled_on_max: null },
        };
        setCancelledClick("");
  ...
  return (
    <StyledOrderFilter>
      <div className="order-filter">
        <Form onFinish={handleFinish} form={form} initialValues={params}>
          ...
          {isShowButtonRemoveSaveSearch && (
            <div style={{ marginRight: "10px" }}>
              <Button icon={<DeleteFilled />} onClick={deleteSaveSearch}></Button>
            </div>
          )}
        </Form>
        ...
        <BaseFilter
          ...
          width={1000}
        >
          <Form onFinish={handleFinish} ref={formRef} initialValues={params} layout="vertical">
                ...
                <Col span={24}>
                  <Row gutter={24}>
                    ...
                    <Col span={8}>
                      ...
                      <Item className="select-sub-status" name="sub_status_code">
                        <Select
                          mode="multiple"
                          showSearch
                          showArrow
