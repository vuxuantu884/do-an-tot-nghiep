...
const OrderSyncFilter = (props: OrderSyncFilterProps) => {
  ...
  const actionList = (
    <Menu>
      <Menu.Item
        key="orrder-sync"
        onClick={() => handleDownloadOrderSelected()}
  ...
  useEffect(() => {
    ...
    if (params.source_id && params.source_id > 0) {
      const source = sourceList?.find(
        (source) => source.id.toString() === params.source_id?.toString(),
      );
    ...
    if (params.connected_status) {
      const connectStatus = CONNECTED_STATUS.find((item) => item.value === params.connected_status);
      filters.push({
  ...
  const handleRemoveTag = (e: any, tag: any) => {
    ...
    if (tag.key === "created_date") {
      newParams = {
        ...newParams,
        ...{ created_date_from: null, created_date_to: null },
      };
      setCreatedClick("");
