...
type ProgressImportFileModalType = {
  visible: boolean;
...
const ProgressImportFileModal: React.FC<ProgressImportFileModalType> = (
  props: ProgressImportFileModalType,
) => {
  ...
  useEffect(() => {
    if (progressData?.errors_msg) {
      const errorList = progressData?.errors_msg.slice(0).split("\n");
      const checkErrorList = errorList.filter((item: any) => item !== "");
      setErrorData(checkErrorList);
    }
  }, [progressData?.errors_msg]);

  const onOkProgressImportFileModal = () => {
    onOk && onOk();
  };

  const onCancelProgressImportFileModal = () => {
    onCancel && onCancel();
  };
  ...
  return (
    <Modal
      ...
      okText="Xác nhận"
      cancelText="Hủy"
      onCancel={onCancelProgressImportFileModal}
      closable={false}
      maskClosable={false}
      footer={
        <StyledModalFooter>
          {isDownloading ? (
            <Button danger onClick={onCancelProgressImportFileModal}>
              Hủy
            </Button>
          ) : (
            <div />
          ...
          <Button type="primary" onClick={onOkProgressImportFileModal} loading={isDownloading}>
            Xác nhận
          </Button>
        </StyledModalFooter>
      }
...
export default ProgressImportFileModal;
