...
const InventoryAdjustmentFilters: React.FC<InventoryAdjustmentFilterProps> = (props: InventoryAdjustmentFilterProps) => {
  ...
  const [isFromInventoryAdjustmentDate, setIsFromInventoryAdjustmentDate] = useState(initialValues.from_inventoryadjustment_date ? moment(initialValues.from_inventoryadjustment_date, "DD-MM-YYYY") : null);
  const [isToInventoryAdjustmentDate, setIsToInventoryAdjustmentDate] = useState(initialValues.to_inventoryadjustment_date ? moment(initialValues.to_inventoryadjustment_date, "DD-MM-YYYY") : null);
  ...
  const clickOptionDate = useCallback(
    ...
    }, [createDateClick, transferDateClick]);
    ...
    (e, tag) => {
      ...
      switch (tag.key) {
        ...
        case 'transfer_date':
          ...
          setIsFromInventoryAdjustmentDate(null)
          setIsToInventoryAdjustmentDate(null)
          onFilter && onFilter({ ...params, from_inventory_adjustment_date: null, to_inventory_adjustment_date: null });
          break; 
        default: break
      }
    },
  ...
  const onFinish = useCallback(
    (values) => {
      ...
      const valuesForm = {
        ...
        from_inventory_adjustment_date: isFromInventoryAdjustmentDate ? moment(isFromInventoryAdjustmentDate, 'DD-MM-YYYY')?.format('DD-MM-YYYY') : null,
        to_inventory_adjustment_date: isToInventoryAdjustmentDate ? moment(isToInventoryAdjustmentDate, 'DD-MM-YYYY').format('DD-MM-YYYY') : null,

      }
      onFilter && onFilter(valuesForm);
    },
    [isFromCreatedDate, isFromInventoryAdjustmentDate,isToInventoryAdjustmentDate, isToCreatedDate, onFilter]
  );
  ...
  let filters = useMemo(() => {
    ...
    if (initialValues.status.length) {
      ...
      initialValues.status.forEach(statusValue => {
        const status = STATUS_INVENTORY_ADJUSTMENT_ARRAY?.find(status => status.value === statusValue)
