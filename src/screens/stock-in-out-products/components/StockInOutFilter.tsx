...
import { StockInOutAdvancedFilter, StockInOutAdvancedFilterMapping, StockInOutBaseFilter, StockInOutBaseFilterMapping, StockInOutOthersType, StockInOutStatus, StockInOutStatusMapping, StockInOutTypeMapping, stockInReason, StockInReasonMappingField, stockOutReason, StockOutReasonMappingField } from "../constant";
...
import { DATE_FORMAT, formatDateFilter, formatDateTimeFilter, splitDateRange } from "utils/DateUtils";
...
const StockInOutFilter: React.FC<StockInOutFilterProps> = (props: StockInOutFilterProps) => {
  ...
  const formatDateRange = (from: Date, to: Date) => {
    if (!from && !to) return
    return `${from ? moment(from).utc(false).format(DATE_FORMAT.DDMMYY_HHmm) : '??'} ~ ${to ? moment(to).utc(false).format(DATE_FORMAT.DDMMYY_HHmm) : '??'}`
  }
  ...
  useEffect(() => {
    ...
    const formatted = formatFieldTag(rest, { ...StockInOutBaseFilterMapping, ...StockInOutAdvancedFilterMapping })
    const transformParams = formatted.map((item) => {
      switch (item.keyId) {
        ...
        case StockInOutBaseFilter.type:
          return { ...item, valueName: `Phiếu ${StockInOutTypeMapping[item.valueId]}` }
        case StockInOutAdvancedFilter.status:
          let statuses: any = [];
          if (isArray(item.valueId)) {
            item.valueId.forEach((item: any) => { statuses.push(StockInOutStatusMapping[item]) })
          } else {
        ...
        case StockInOutAdvancedFilter.stock_in_reasons:
          if (isArray(item.valueId)) {
            const stockInReason = item.valueId.map((el: any) => { return StockInReasonMappingField[el] })
            return { ...item, valueName: stockInReason.toString() }
          } else {
            return { ...item, valueName: StockInReasonMappingField[item.valueId] }
          }
        case StockInOutAdvancedFilter.stock_out_reasons:
          if (isArray(item.valueId)) {
            const stockOutReason = item.valueId.map((item: any) => { return StockOutReasonMappingField[item] })
            return { ...item, valueName: stockOutReason.toString() }
          } else {
            return { ...item, valueName: StockOutReasonMappingField[item.valueId] }
          }
        default:
          return item
      }
  ...
  useEffect(() => {
    ...
    formBase.setFieldsValue({
      [StockInOutBaseFilter.stores]: paramsUrl.stores?.toString()?.split(',').map((x: string) => parseInt(x)),
    ...
    (async () => {
      if (paramsArray.length < (prevArray?.length || 0)) {
        ...
        if (newParams?.created_date) {
          ...
          newParams = {
            ...newParams,
            created_date_from: start !== "??" ? formatDateTimeFilter(start, 'DD/MM/YYYY HH:mm')?.format(): undefined,
            created_date_to: end !== "??" ? formatDateTimeFilter(end, 'DD/MM/YYYY HH:mm')?.format(): undefined,
          }
        }
  ...
  const onSearch = debounce((value: string) => {
    let queryParam = generateQuery({ ...paramsUrl, page: 1, [StockInOutBaseFilter.content]: value });
    history.replace(`${UrlConfig.STOCK_IN_OUT_OTHERS}?${queryParam}`);
  }, 300)
            ...
            {Object.values(StockInOutAdvancedFilter).map((field) => {
              ...
              switch (field) {
                ...
                case StockInOutAdvancedFilter.stock_out_reasons:
                  ...
                  break
                case StockInOutAdvancedFilter.account_codes:
                  component = <AccountSearchPaging mode="multiple" placeholder="Chọn người đề xuất" />;
                  break;
                case StockInOutAdvancedFilter.created_by:
              ...
              return (
                <Col span={12} key={field}>
                  ...
                  {field === StockInOutAdvancedFilter.internal_note
                    ? <Item name={field} rules={[{ max: 255, message: "Thông tin tìm kiếm không được quá 255 ký tự" }]}>{component}</Item>
