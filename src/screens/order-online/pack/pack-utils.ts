...
export const getFullfilmentReturning = (fulfillment: FulFillmentResponse[]) => {
    let fulfillmentResult = fulfillment.filter(p => p.status === FulFillmentStatus.SHIPPING && p.return_status === FulFillmentStatus.RETURNING)
    return fulfillmentResult;
}
...
export const isFulfillmentShiping = (fulfillment: FulFillmentResponse | any) => {
    let fulfillmentBol = (fulfillment.status === FulFillmentStatus.SHIPPING || fulfillment.status === FulFillmentStatus.SHIPPED)
...
export const isFullfilmentReturning = (fulfillment: FulFillmentResponse | any) => {
    let fulfillmentBol = fulfillment.status === FulFillmentStatus.SHIPPING && fulfillment.return_status === FulFillmentStatus.RETURNING;
...
export const isFullfilmentReturned = (fulfillment: FulFillmentResponse | any) => {
    let fulfillmentBol = fulfillment.status === FulFillmentStatus.CANCELLED
