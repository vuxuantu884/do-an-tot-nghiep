...
const ButtonWarningHandover: React.FC<ButtonCreateProps> = (
    ...
    const renderTitleButtonCreate = useMemo(() => {
        ...
        return <div>Đã đóng gói chưa thêm vào biên bản <span style={{ color: "red" }}>({orderQuantity})</span></div>
    }, [orderPackedNotRecord?.metadata.total])

    const renderTitleModel = useMemo(() => {
        const orderQuantity: number = orderPackedNotRecord?.metadata.total || 0;
        return <div>Danh sách các đơn đã đóng gói chưa thêm vào biên bản <span style={{ color: "red" }}>({orderQuantity})</span></div>
    }, [orderPackedNotRecord?.metadata.total])
                    ...
                    <Modal
                        ...
                        footer={
                            <Space size={12} style={{ marginLeft: "10px" }}>
                                <Button type="default" size="small" onClick={() => setIsVisible(false)}>Bỏ qua</Button>
                                ...
                                columns={[
                                    {
                                        ...
                                        align: "center",
                                        key: "code",
                                        render: (value: any, record: OrderModel, index: number) => <Link to={`${UrlConfig.ORDER}/${record.id}`} target="_blank">{record.code}</Link>
                                    },
                                    {
                                        title: "Mã đơn giao",
                                        ...
                                        render: (value: any, record: OrderModel, index: number) => {
                                            ...
                                            return (
                                                <Link to={`${UrlConfig.ORDER}/${record.id}`} target="_blank">{fulfillment?.code}</Link>
                                            )
                                        }
                                    },
                                    {
                                        ...
                                        render: (value: any, record: OrderModel, index: number) => {
                                            return (
                                                <React.Fragment>
                                                    <Link to={`${UrlConfig.STORE}/${record.store_id}`} target="_blank">{record.store}</Link>
