...
const PackConfirmModal = (props: Props) => {
    const { isVisible = false, setIsVisible, orderPushFalseDelivery, setOrderPushFalseDelivery } = props;
    ...
    const handleRemoveFulfillment = useCallback((fulfillmentId: number) => {
        ...
        const index1 = orderPushFalseDeliveryCopy.findIndex(p => p.fulfillment_active?.id === fulfillmentId);
        if (index !== -1) {
            ...
            setPackInfo({ ...new PackModelDefaultValue(), ...singlePack, fulfillments: fulfillmentsCopy });
        ...
        if (orderPushFalseDelivery) {
            let result: orderPushFalseDeliveryTable[] = orderPushFalseDelivery.map(p => {
                return {
                    ...
                    delivery_service_name: p.fulfillment_active?.shipment?.delivery_service_provider_name || "",
                    ...
                    columns={[
                        {
                            ...
                            render: (value: any, record: orderPushFalseDeliveryTable,) => (
                                <Link to={`${UrlConfig.ORDER}/${record.order_id}`} target="_blank">{value}</Link>
                            )
                        },
                        {
                            title: 'Mã đơn giao',
                        ...
                        },
                        {
                            ...
                            width: 100,
                            render: (value: any, record: orderPushFalseDeliveryTable, index: number) => (
                                <Button style={{ height: "30px" }} type="primary" loading={record.is_loading} onClick={() => { handlePushFulfillment(record.fulfillment_id) }}>Đẩy lại đơn</Button>
                            )
                        },
                        {
                            title: '',
                            ...
                            render: (value: any, record: orderPushFalseDeliveryTable, index: number) => (
                                ...
                                <Popconfirm
                                    ...
                                    placement="leftTop"
                                >
                                    <Button style={{ width: "30px", height: "30px", padding: 0 }} icon={<CloseOutlined />} />
                                </Popconfirm>
                            )
                        }
                    ]}
