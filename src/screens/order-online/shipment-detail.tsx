...
const ShipmentDetail: React.FC = () => {
  ...
  const requirement = useMemo(() => {
    const findRequirement = shipping_requirement?.find(
      (item) => item.value === fulfillmentDetail?.shipment?.requirements,
    );
  ...
  const [columnsItems, setColumnsItems] = useState<Array<any>>([
    ...
    },
    {
      ...
      render: (quantity: number) => (
        <div>
          <NumberFormat value={formatNumber(quantity)} className="foo" displayType={"text"} />
    ...
    },
    {
      title: (
        <span>
          <span>Đơn giá </span>
          <span style={{ color: "#737373", fontWeight: 400 }}> đ</span>
        </span>
      ),
      dataIndex: "price",
      render: (price: number) => (
        <div>
          <NumberFormat value={formatCurrency(price)} className="foo" displayType={"text"} />
          <div style={{ height: 40 }}></div>
        </div>
      ),
      visible: true,
      ...
      render: (record: any) => (
        <div>
          <NumberFormat
            ...
            displayType={"text"}
          />
          <div style={{ color: "#E24343", height: 40 }}>
            {formatPercentage(record.discount_rate)}%
          </div>
        </div>
      ),
      visible: true,
  ...
  const onOKGoodsReturn = useCallback(() => {
    ...
    dispatch(
      UpdateFulFillmentStatusAction(value, () => {
        showSuccess("Nhận hàng thành công");
  ...
  const rePush = useCallback(() => {
    ...
    dispatch(
      RePushFulFillmentAction(
        fulfillment_id,
        (response) => {
          showSuccess("Đẩy đơn sang HVC thành công");
          setFulfillmentDetail({
            ...fulfillmentDetail,
            shipment: {
  ...
  useEffect(() => {
    dispatch(
      getFulfillmentDetail(code, (data) => {
        const status = status_order.find((status) => status.value === data.status);
    ...
    dispatch(getTrackingLogFulfillmentAction(code, setTrackingLogFulfillment));
  ...
  return (
    <ContentContainer
      ...
      breadcrumb={[
        ...
        {
          name: code ? `Đơn giao hàng ${code}` : "Đang tải dữ liệu...",
        },
      ]}
    >
      <Row gutter={20} className="fulfillment-detail">
              ...
              <div className="title">
                <span>
                  ...
                  <span style={{ color: "#2A2A86" }}>
                    {" "}
                    {fulfillmentDetail?.shipment?.tracking_code}
                  </span>
                </span>
                <span style={{ textTransform: "initial" }}>
                  ...
                  <span
                    style={{
                      color:
                        fulfillmentDetail?.shipment?.pushing_status !== "failed"
                          ? fulfillmentDetail?.status_color
                          : "#E24343",
                      paddingLeft: 5,
                    }}
                  >
                    {fulfillmentDetail?.status !== "cancelled" &&
                    fulfillmentDetail?.shipment?.pushing_status === "failed"
                      ? "Đẩy đơn hvc thất bại"
                      : fulfillmentDetail?.status_name}
                  </span>
                </span>
                <span style={{ minWidth: 100 }}>
                  {fulfillmentDetail?.status !== "cancelled" &&
                    fulfillmentDetail?.shipment?.pushing_status === "failed" && (
                      <Button type="primary" onClick={() => rePush()} loading={loading1}>
                        Đẩy lại đơn HVC
                      </Button>
                    )}
                  {fulfillmentDetail?.return_status === "returning" && (
                    <Button type="primary" onClick={() => onOKGoodsReturn()} loading={loading}>
                      Nhận hàng
                    </Button>
                  )}
                </span>
          ...
          >
            <div className="details">
              <div className="detail">
                ...
                <span className="value" style={{ color: "#2A2A86" }}>
                  ...
                  <Link
                    target="_blank"
                    to={`${UrlConfig.ORDER}/${fulfillmentDetail?.order?.id}`}
              ...
              <div className="detail">
                ...
                <span className="value">
                  {fulfillmentDetail?.order?.shipping_address?.district}
                </span>
              </div>
              <div className="detail">
            ...
            <div className="details">
              <div className="detail">
                ...
                <span className="value" style={{ color: "#2A2A86" }}>
                  <b>{fulfillmentDetail?.order?.store}</b>
                </span>
              </div>
              <div className="detail">
              ...
              <div className="detail">
                ...
                <span className="value">
                  {fulfillmentDetail?.picked_on
                    ? ConvertUtcToLocalDate(fulfillmentDetail?.picked_on)
                    : "-"}
                </span>
              </div>
              <div className="detail">
                ...
                <span className="value">
                  {fulfillmentDetail?.packed_on
                    ? ConvertUtcToLocalDate(fulfillmentDetail?.packed_on)
                    : "-"}
                </span>
              </div>
              <div className="detail">
                ...
                <span className="value">
                  {fulfillmentDetail?.export_on
                    ? ConvertUtcToLocalDate(fulfillmentDetail?.export_on)
                    : "-"}
                </span>
              </div>
              <div className="detail">
                ...
                <span className="value">
                  {fulfillmentDetail?.cancel_date
                    ? ConvertUtcToLocalDate(fulfillmentDetail?.cancel_date)
                    : "-"}
                </span>
              </div>
              <div className="detail">
                ...
                <span className="value">
                  {fulfillmentDetail?.received_on
                    ? ConvertUtcToLocalDate(fulfillmentDetail?.received_on)
                    : "-"}
                </span>
              </div>
            </div>
              ...
              >
                ...
                <span style={{ width: "20%", padding: 16, fontWeight: 700 }}>
                  <span style={{ float: "right" }}>
                    <NumberFormat
                      value={formatNumber(
                        itemTotal.afterDiscountTotal ? itemTotal.afterDiscountTotal : 0,
                      )}
                      className="foo"
                      displayType={"text"}
                    />
              ...
              <div style={{ width: "50%", padding: 16 }}>
                  ...
                  <div>
                    <NumberFormat
                      value={formatCurrency(
                        fulfillmentDetail.total_line_amount_after_line_discount,
                      )}
                      className="foo"
                      displayType={"text"}
                    />
                ...
                >
                  ...
                  <div>
                    <NumberFormat
                      value={formatCurrency(
                        fulfillmentDetail.discount_amount ? fulfillmentDetail.discount_amount : 0,
                      )}
                      className="foo"
                      displayType={"text"}
                    />
                ...
                >
                  <div>Mã giảm giá:</div>
                  <div>
                    <NumberFormat value={0} className="foo" displayType={"text"} />
                  </div>
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  <div>
                    <NumberFormat
                      value={formatCurrency(
                        fulfillmentDetail?.shipment?.shipping_fee_informed_to_customer
                          ? fulfillmentDetail.shipment.shipping_fee_informed_to_customer
                          : 0,
                      )}
                      className="foo"
                ...
                >
                  ...
                  <div style={{ color: "#2A2A86", fontSize: 18, fontWeight: 700 }}>
                    <NumberFormat
                      value={formatCurrency(
                        fulfillmentDetail?.order?.total ? fulfillmentDetail.order.total : 0,
                      )}
                      className="foo"
                      displayType={"text"}
                    />
        ...
        <Col md={8}>
          <Card title="THÔNG TIN ĐƠN GIAO HÀNG" className="info">
            <div className="detail">
              <span className="name">Hãng vận chuyển:</span>
              <span className="value">
                {fulfillmentDetail?.shipment?.delivery_service_provider_name}
              </span>
            </div>
            <div className="detail">
            ...
            <div className="detail">
              ...
              <span className="value">
                <NumberFormat
                  value={formatCurrency(
                    fulfillmentDetail?.shipment?.shipping_fee_paid_to_three_pls,
                  )}
                  className="foo"
                  displayType={"text"}
                />
            ...
            <div className="detail">
              ...
              <span className="value">
                {fulfillmentDetail?.created_date
                  ? ConvertUtcToLocalDate(fulfillmentDetail?.created_date)
                  : "-"}
              </span>
            </div>
            <div className="detail">
              ...
              <span className="value">
                {fulfillmentDetail?.shipment?.expected_received_date
                  ? ConvertUtcToLocalDate(fulfillmentDetail?.shipment.expected_received_date)
                  : "-"}
              </span>
            </div>
            <div className="detail">
