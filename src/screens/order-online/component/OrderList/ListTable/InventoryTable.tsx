...
const InventoryTable: React.FC<InventoryTableProps> = (props: InventoryTableProps) => {
  ...
  const data = useMemo(() => {
    ...
    listStore?.forEach((value, index) => {
      ...
      items?.forEach((p) => {
        let inventory = inventoryData.find(
          (p1) => p.variant_id === p1.variant_id && value.id === p1.store_id,
        );
        store.data[p.variant_id.toString()] =
      ...
      <Col span={24}>
        <StyledComponent>
          <div
            ...
            style={{ overflowX: "auto", WebkitScrollSnapPointsY: "2px" }}
          >
                ...
                {data?.map((item, index) => (
                  <tr key={index}>
                    <th className="condition" key={index}>
                      {item.name}{" "}
                      <CheckCircleOutlined
                        hidden={item.id === storeId ? false : true}
                        style={{ color: "#2A2A86" }}
                      />
                    </th>
                    ...
                    {items?.map((_itemi, index) => (
                      <td
                        className={`condition ${item.data[_itemi.variant_id] <= 0 ? "red" : null}`}
