...
type PropTypes = {
  ...
  setListReturnProducts:
    | ((listReturnProducts: ReturnProductModel[]) => void)
    | undefined;
...
function CardReturnProducts(props: PropTypes) {
  ...
  const [isStoreReturnModalVisible, setStoreReturnModalVisible] =
    useState(false);
  ...
  const columns: ColumnType<any>[] = [
    ...
    },
    {
      ...
      render: (value: number, record: ReturnProductModel, index: number) => {
        ...
        const discountPerOrder = getProductDiscountPerOrder(
          OrderDetail,
          record,
        );
        const pricePerOrder =
  ...
  const dataCanAccess = useMemo(() => {
    ...
    if (newData && newData[0]?.id) {
      if (!storeReturn) {
        if (storeIdLogin) {
          const newStoreIndex = listStores.findIndex(
            (p) => p.id === storeIdLogin,
          );
            ...
            <Col md={8}>
              <CustomSelect
                ...
                onChange={(value?: number) => {
                  if (value) onChangeStoreReturrn(value);
                }}
              >
