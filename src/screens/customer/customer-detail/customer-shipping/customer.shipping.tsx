...
const CustomerShippingAddressInfo: React.FC<CustomerShippingAddressInfoProps> = (
  ...
  const shippingColumns: Array<ICustomTableColumType<ShippingAddress>> = [
    ...
    // },
    {
      ...
      render: (value, row, index) => {
        return (
          <div>
            <div className="text" title={row.code} style={{ color: "#666666", width: 300 }}>
  ...
  const handleShippingAddressForm = {
    create: (formValue: CustomerShippingAddress) => {
      ...
      if (customer)
        dispatch(
          CreateShippingAddress(customer.id, formValue, (data: ShippingAddress) => {
    ...
    <Row>
      <Col span={24}>
        ...
        <CustomerModal
          ...
          onEdit={(formValue: CustomerShippingAddress) => handleShippingAddressForm.edit(formValue)}
          onDelete={() => {}}
