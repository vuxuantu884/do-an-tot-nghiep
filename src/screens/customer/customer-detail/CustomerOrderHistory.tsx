...
import { CustomerOrderHistoryResponse, DeliveryServiceResponse, ShipmentResponse, TrackingLogFulfillmentResponse } from "model/response/order/order.response";
...
import { checkIfOrderCanBeReturned, copyTextToClipboard, formatCurrency, getOrderTotalPaymentAmount, getTotalQuantity, handleFetchApiError, isFetchApiSuccessful } from "utils/AppUtils";
...
import { getFulfillmentActive, getLink, getReturnMoneyStatusColor, getReturnMoneyStatusText } from "utils/OrderUtils";
...
const CustomerOrderHistory: React.FC<Props> = (props: Props) => {
    ...
    const [orderHistoryQueryParams, setOrderHistoryQueryParams] = useState<OrderHistorySearch>(initOrderSearchingQuery);
    const [keySearchVariant, setKeySearchVariant] = useState("");

    const [purchaseHistoryData, setPurchaseHistoryData] = useState<PageResponse<CustomerOrderHistoryResponse>>({
        metadata: {
            limit: 10,
            page: 1,
            total: 0,
    ...
    useEffect(() => {
        if (customer?.id) {
            ...
            setIsLoading(true);
            getOrderHistoryService(query).then((response) => {
    ...
    const columnsPurchaseHistory: Array<ICustomTableColumType<CustomerOrderHistoryResponse>> = useMemo(() =>
        [
            {
                title: "Khách hàng",
                visible: true,
                    ...
                    return (
                        <div>
                                ...
                                {isOrderReturn ?
                                    <>
                                        <Tooltip title="Mã đơn trả hàng">
                                            <Link to={`${UrlConfig.ORDERS_RETURN}/${item.id}`} target="_blank">{item.code}</Link>
                                        </Tooltip>
                            ...
                            <div style={{ display: 'flex' }}>
                                    ...
                                    <div style={{ display: 'block' }}>
                                        <div className="textSmall">
                                            ...
                                            <Tooltip title="Mã đơn gốc">
                                                <Link to={`${UrlConfig.ORDER}/${item.order_id}`} target="_blank">{item.order_code}</Link>
                                            </Tooltip>
                                        </div>
                                ...
                                {(item.status === OrderStatus.FINISHED || item.status === OrderStatus.COMPLETED) && !isOrderReturn ? (
                ...
                render: (record: CustomerOrderHistoryResponse) => {
                    ...
                    return (
                        <React.Fragment>
                            {isOrderReturn ?
                                <div>
                                    <div className="plus-point">{`Trừ Tích: ${record.change_point?.subtract ? record.change_point?.subtract : 0}`}</div>
                                    <div className="minus-point">{`Hoàn Tiêu: ${record.change_point?.add ? record.change_point?.add : 0}`}</div>
                                </div>
                                :
                                <div>
                                    <div className="plus-point">{`Tích: ${record.change_point?.add ? record.change_point?.add : 0}`}</div>
                                    <div className="minus-point">{`Tiêu: ${record.change_point?.subtract ? record.change_point?.subtract : 0}`}</div>
                                    {record.status === OrderStatus.CANCELLED &&
                                        <div className="minus-point">{`Hoàn tiêu: ${record.change_point?.subtract ? record.change_point?.subtract : 0}`}</div>
                                    }
                                </div>
                            }
                        </React.Fragment>
        ...
        [deliveryServices, editNote, renderOrderPayments, renderOrderReturnPayments, status_order, subStatuses]
    );
    ...
    const onFinish = useCallback((value) => {
        console.log(value)
        const newParams: OrderHistorySearch = { ...orderHistoryQueryParams, page: 1, search_term: value.search_term, search_product: keySearchVariant }
        setOrderHistoryQueryParams(newParams)
    }, [keySearchVariant, orderHistoryQueryParams])

    const handleSelectProduct = useCallback((v?: VariantResponse) => {
        if (v) {
            const newParams: OrderHistorySearch = { ...orderHistoryQueryParams, page: 1, search_product: v.sku }
            setOrderHistoryQueryParams(newParams)
        }
    }, [orderHistoryQueryParams])
        ...
        <StyledPurchaseHistory>
            <Spin spinning={isLoading}>
                <Row className="customer-order-history-filter">
                    <Col md={24}>
                        <Form onFinish={onFinish} ref={formSearchRef} layout="inline" className="customer-order-history-filter-form">
