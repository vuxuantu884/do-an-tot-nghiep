...
const UploadLoyaltyCardRelease = () => {
  ...
  const getProgressImportFile = useCallback(() => {
    let getImportProcessPromise: Promise<BaseResponse<any>> =
      getLoyaltyCardReleaseJobsApi(processId);

    Promise.all([getImportProcessPromise])
      .then((responses) => {
            ...
            if (response.data && response.data.finish) {
              ...
              if (response.data?.total > 0) {
                const percent = Math.floor(
                  ((response.data.total_success + response.data.total_error) /
                    response.data.total) *
                    100,
                );
            ...
            <Row style={{ marginBottom: "20px" }}>
              <Col span={10}>
                <div className="release-label">
                  Tên đợt phát hành <span className="text-error">*</span>
                </div>
              ...
              <Col span={14}>
                <div className="sample-file">
                  File mẫu tạo đợt phát hành: Click để tải{" "}
                  <a
                    href="https://loyalty-resource.s3.ap-southeast-1.amazonaws.com/sample.xlsx"
                    rel="noreferrer"
                    target="_blank"
                  >
                    “File excel mẫu”
              ...
              <Col span={24}>
                <div className="upload-area">
                  <span className="title">
                    File excel <span className="text-error">*</span>
                  </span>
          ...
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <div style={{ marginLeft: 15 }}>
              <strong style={{ fontSize: 16 }}>
                Bạn có chắc chắn muốn hủy tiến trình tạo mới đợt phát hành không?
              </strong>
            </div>
          </div>
        </Modal>
