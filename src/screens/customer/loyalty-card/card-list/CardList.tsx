...
import {
  LoyaltyCardLock,
  LoyaltyCardSearch,
} from "domain/actions/loyalty/card/loyalty-card.action";
...
const LoyaltyCards = () => {
  ...
  const pageColumns: Array<ICustomTableColumType<any>> = [
    ...
    },
    {
      ...
      render: (value: any) => (
        <div>
          {value.assigned_date && moment(value.assigned_date).format(DATE_FORMAT.DDMMYY_HHmm)}
        </div>
      ),
    },
    {
  ...
  const [params, setParams] = useState<CustomerCardListRequest>({
    ...initParams,
  });
