...
} from "screens/web-app/orders/orderStyles";
...
const LogisticConfirmModal: React.FC<LogisticConfirmModalProps> = (
  ...
  const handleConfirmToEcommerce = () => {
    ...
    dispatch(
      createEcommerceLogistic(request, (data) => {
        if (!data.error) {
    ...
    if (ecommerceStoreAddress) {
      ...
      if (currentStore && currentStore.time_slot_list.length > 0) {
        ...
        storeAddressForm.setFieldsValue({
          picking_time:
            currentStore.time_slot_list.length > 0
              ? currentStore.time_slot_list[0].date
              : "Đơn hàng đang giao hoặc đã bị hủy trên sàn",
        });
      }
    }
  };
  ...
  useEffect(() => {
    if (ecommerceStoreAddress) {
      ...
      if (defaultStore) {
        ...
        if (defaultStore.time_slot_list.length > 0)
          setPickupTime(defaultStore.time_slot_list[0].date);
        storeAddressForm.setFieldsValue({
          address: defaultStore.address_id,
          picking_time:
            defaultStore.time_slot_list.length > 0
              ? defaultStore.time_slot_list[0].date
              : "Đơn hàng đang giao hoặc đã bị hủy trên sàn",
        });
      }
    }
  ...
  return (
    <StyledLogisticConfirmModal>
      ...
      >
        <StyledDownloadOrderData>
          <Form layout="vertical" form={storeAddressForm}>
            <Form.Item label="Ngày bàn giao" name="picking_time">
              <Select
            ...
            <Form.Item label="Địa chỉ lấy hàng" name="address">
              ...
              >
                {ecommerceStoreAddress &&
                  ecommerceStoreAddress?.map((item) => (
