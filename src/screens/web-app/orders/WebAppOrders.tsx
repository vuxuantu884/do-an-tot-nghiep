...
const WebAppOrders: React.FC = () => {
  ...
  const onOkExitProcessModal = () => {
    ...
    if (commonProcessId) {
      dispatch(
        exitWebAppJobsAction(commonProcessId, (responseData) => {
          if (responseData) {
  ...
  const handleProgress = useCallback(() => {
    ...
    let getProgressPromises: Promise<BaseResponse<any>> = getWebAppJobsApi(commonProcessId);
  ...
  const actionList = [
    ...
    //   name: "In phiếu giao hàng website",
    //   icon: <PrinterOutlined />,
    //   disabled: !selectedRowKeys?.length || !data.items.length,
  ...
  const callbackDownloadWebAppOrders = (data: any) => {
    setIsLoading(false);
    if (data) {
      setIsShowGetOrderModal(false);
  ...
  const handleDownloadOrdersSync = (params: any) => {
    setIsLoading(true);
    dispatch(downloadWebAppOrderAction(params, callbackDownloadWebAppOrders));
  };
