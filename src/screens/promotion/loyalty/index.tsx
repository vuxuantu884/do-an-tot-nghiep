...
const LoyaltyPage = () => {
  ...
  const [loyaltyPrograms, setLoyaltyPrograms] = useState<
    PageResponse<LoyaltyAccumulationProgramResponse>
  >({
    metadata: {
      limit: 30,
      page: 1,
  ...
  const columns: Array<ICustomTableColumType<any>> = [
    {
      ...
      render: (value: any, item: any, index: number) => (
        <div>
          {(loyaltyPrograms.metadata.page - 1) * loyaltyPrograms.metadata.limit + index + 1}
        </div>
      ),
      width: "72px",
    },
    ...
    },
    {
      ...
      align: "center",
      render: (value: any, item: any, index: number) => (
        <div className={`status-col__${value.status}`}>
          {value.status === "INACTIVE" ? "Tạm dừng" : "Đang chạy"}
        </div>
      ),
    },
    {
  ...
  useEffect(() => {
    dispatch(
      getLoyaltyRate((rates: LoyaltyRateResponse) => {
    ...
    dispatch(
      getLoyaltyUsage((data: Array<LoyaltyUsageResponse>) => {
        let _rules = [];
  ...
  const onFinish = useCallback(() => {
    dispatch(
      createLoyaltyRate(accumulationRate, redemptionRate, enableUsingPoint, updateRateCallback),
    );
      ...
      <div className="loyalty-programs-wrapper">
        {allowConfigLoyalty && (
          ...
          >
            <div className="general-setting__body">
              <Row className="conversion-rate">
                <Col span={10}>
                  ...
                  <div className="d-flex">
                    ...
                    />
                    <span className="conversion-note" style={{ marginLeft: "12px" }}>
                      = 1 điểm thưởng
                    </span>
                  </div>
                </Col>
                <Col span={14}>
                  ...
                  <div className="d-flex">
                    <span className="conversion-note" style={{ marginRight: "12px" }}>
                      1 điểm thưởng ={" "}
                    </span>
                    <CurrencyInput
                      position="after"
                  ...
                  <div className="redemption-rules__table">
                    <table className="rules">
                      ...
                      <tbody>
                          ...
                          {rules.map((rule, index) => (
                            <td className="condition checkbox" key={index}>
                              <Checkbox
                                ...
                                onChange={(e) =>
                                  onChangePreventDiscountOrder(e.target.checked, index)
                                }
                              />
                            </td>
                          ))}
                        ...
                        <tr>
                          <td className="condition">
                            Số điểm có thể tiêu không quá ... giá trị hóa đơn
                          </td>
                          {rules.map((rule, index) => (
                            <td className="condition" key={index}>
                              <CurrencyInput
                                ...
                                onChangeCurrencyType={(value) =>
                                  handleChangePointUseType(value, index)
                                }
                        ...
                        <tr>
                          <td className="condition">
                            Tích điểm với các sản phẩm có chiết khấu dưới ... %
                          </td>
                          {rules.map((rule, index) => (
                            <td className="condition" key={index}>
                              <CurrencyInput
              ...
              <Row>
                <Button type="primary" className="save-btn" onClick={onFinish}>
                  Lưu cài đặt
                </Button>
              </Row>
            </div>
        ...
        {allowViewProgramList && (
          <Card
            title={
              <div className="d-flex">
                <span className="tab-label">Danh sách chương trình</span>
                {allowCreateProgram && (
        ...
        {!allowConfigLoyalty && !allowViewProgramList && <NoPermission />}
      </div>
    </ContentContainer>
  );
};
