...
import {
  LoyaltyAccumulationProgramResponse,
  LoyaltyProgramRuleResponse,
} from "model/response/loyalty/loyalty-accumulation.response";
...
const LoyaltyAccumulateDetail = () => {
  ...
  const columns: Array<ICustomTableColumType<any>> = [
    ...
    },
    {
      ...
      visible: true,
      render: (rule: LoyaltyProgramRuleResponse) => {
        return <span>{types.filter((type) => rule.customer_type_id === type.id)[0]?.name}</span>;
      },
    },
    {
      ...
      visible: true,
      render: (rule: LoyaltyProgramRuleResponse) => {
        return <span>{ranks.filter((rank) => rule.customer_ranking_id === rank.id)[0]?.name}</span>;
      },
    },
    {
  ...
  return (
    ...
    >
            ...
            <Card
              title={
                <div className="d-flex">
                  <span className="title-card">Thông tin chương trình</span>
                  {allowUpdateProgram && (
                    <Link
              ...
              <div className="program-info">
                <Row>
                  ...
                  <Col span={12} className="row-item">
                    ...
                    <div
                      className={`priority priority__${
                        loyaltyProgram?.priority === 1 ? "HIGH" : "LOW"
                      } align-center`}
                    >
                      Số {loyaltyProgram?.priority}
                    </div>
                  </Col>
                </Row>
                <Row>
                  <Col span={12} className="row-item">
                    ...
                    <div className={`info-status info-status__${loyaltyProgram?.status}`}>
                      {loyaltyProgram?.status === "ACTIVE" ? "Đang hoạt động" : "Ngừng hoạt động"}
                    </div>
                  </Col>
                  <Col span={12} className="row-item">
                    <div className="info-label">Sản phẩm:</div>
                    <div className="info-content">
                      {loyaltyProgram?.items.length
                        ? loyaltyProgram.items.length + " sản phẩm đã chọn"
                        : ""}
                      {loyaltyProgram?.items.length ? (
                        <span className="load-products" onClick={() => setIsShowProducts(true)}>
                          Xem chi tiết
                        </span>
                      ) : (
                        <></>
                ...
                <Row>
                  <Col span={12} className="row-item" style={{ marginBottom: "0" }}>
                    ...
                    <div className="info-content">
                      <ul>
                        {loyaltyProgram?.having_card && (
                          <li>Yêu cầu có thẻ khách hàng mới được tích điểm</li>
                        )}
                        {loyaltyProgram?.not_using_point && (
                          <li>Không tích điểm cho hóa đơn có tiêu điểm tích lũy</li>
                        )}
                      </ul>
                    </div>
                  </Col>
                  <Col span={12} className="row-item" style={{ alignSelf: "flex-start" }}>
                    ...
                    <div className="info-content">
                      {loyaltyProgram?.channels.map((channel) => channel.name).join(", ")}
                    </div>
                  </Col>
                </Row>
                <Row>
                  ...
                  <Col span={12} className="row-item">
                    ...
                    <div className="info-content">
                      {ConvertUtcToLocalDate(loyaltyProgram?.start_time, DATE_FORMAT.DDMMYY_HHmm)} -{" "}
                      {ConvertUtcToLocalDate(loyaltyProgram?.end_time, DATE_FORMAT.DDMMYY_HHmm)}
                    </div>
                  </Col>
                </Row>
                <Row>
                    ...
                    {loyaltyProgram && loyaltyProgram.sources && loyaltyProgram.sources.length > 0 && (
                      <div className="scroll-box">
                        {loyaltyProgram?.sources.map((source, idx) => (
                          <div className="scroll-box__item" key={source.id}>
                            {idx + 1}. {source.name}
                          </div>
                        ))}
                      </div>
                    ...
                    {loyaltyProgram && loyaltyProgram.stores && loyaltyProgram.stores.length > 0 && (
                      <div className="scroll-box">
                        {loyaltyProgram?.stores.map((store, idx) => (
                          <div className="scroll-box__item" key={store.id}>
                            {idx + 1}. {store.name}
                          </div>
                        ))}
                      </div>
      ...
      <LoyaltyProgramProducts
        visible={isShowProducts}
        onCancel={() => setIsShowProducts(false)}
        items={loyaltyProgram?.items || []}
      />
    </ContentContainer>
  );
};
