...
function ProductPOItem(props: Props) {
    ...
    return <ListItem item={item} onSelect={(item: PurchaseOrderLineItem, index: number) => handleSelectProduct(item, index, selectedIds, setSelectedIds)} index={index} checked={isSelected} showCheckBox={showCheckBox}>
        ...
        <div className="product-item-info">
            ...
            <div className="product-item-info-right">
                <span className="product-item-price">{typeof price === "number" ? formatCurrency(price) : "-"} <span className="currency">₫</span></span>
                {item?.quantity && <span className="product-item-inventory">Số lượng: <span className="value">{item?.quantity}</span></span>}
            </div>
        </div></ListItem>
