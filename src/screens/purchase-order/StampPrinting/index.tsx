...
import { defaultBreadcrumbStampPrinting, FormFiledStampPrinting, handleChangeQtyPrintByVariantId, handleSupplementalStamp, initFormValueStampPrinting } from './helper';
...
function PrintingStamp() {
  ...
  const notSelectedProducts = (): PurchaseOrderLineItem[] => {
    const selectedProducts: ProductStampPrinting[] = form.getFieldValue(FormFiledStampPrinting.variants) || [];
    return purchaseOrder?.line_items.filter(item => (item.id && !selectedProducts.some(selected => selected.variant_id === item.variant_id))) || [];
  };

  const handleSearchPOVariantsModal = (value: string) => {
    if (value) {
      const resultSearch = notSelectedProducts().filter(item => {
  ...
  const handleSearchPOVariantAutoComplete = (value: string) => {
    if (value) {
      const resultSearch = notSelectedProducts().filter(item => fullTextSearch(value, `${item.sku} ${item.variant}`)) || [];
      setAutoCompleteOptions(resultSearch.map((item, index) => (
        {
          label: <ProductPOItem item={item} index={index} selectedIds={[]} setSelectedIds={() => { }} showCheckBox={false} />,
          value: item.id
        })))
    } else {
      setAutoCompleteOptions(notSelectedProducts().map((item, index) => (
        {
          label: <ProductPOItem item={item} index={index} selectedIds={[]} setSelectedIds={() => { }} showCheckBox={false} />,
          value: item.id
        })))
  ...
  const handleCheckAllProduct = (checked: boolean) => {
    if (checked) {
      const selectIds = modalOptions.list.reduce((acc: number[], item: PurchaseOrderLineItem) => {
        if (item.variant_id) {
  ...
  const handleSelectProductAutoComplete = (value: string) => {
    ...
    if (lineItem && !selectedProduct.some(item => item.variant_id === lineItem.product_id)) {
      ...
      form.setFieldsValue({ [FormFiledStampPrinting.variants]: [...selectedProduct] });
      updateOptions()
    }
  }
  ...
  useEffect(() => {
    if (poId) {
      dispatch(PoDetailAction(poId, (po) => {
        if (po) {
          ...
          setPurchaseOrder(po);
          setBreadcrumb([...defaultBreadcrumbStampPrinting, { name: po.code, path: `${UrlConfig.PURCHASE_ORDERS}/${po.id}` }, { name: 'In mã vạch' }])
      ...
      <StampPrintingStyle>
        <Form form={form} initialValues={initFormValueStampPrinting} onFinish={handleFinish}>
          <Card>
            ...
            <Row gutter={10} className="row-action">
              ...
              <Col md={8} lg={10} xl={12}>
                <Item label="Ghi chú" labelCol={{ span: 24 }} help={false} name={FormFiledStampPrinting.note}>
                ...
                return <Table
                  ...
                  columns={[
                    ...
                    },
                    {
                      ...
                      render: (text, record) => {
                        if (record.product_id && record.variant_id) {
                          return <a href={`${BASE_NAME_ROUTER}${UrlConfig.PRODUCT}/${record.product_id}${UrlConfig.VARIANTS}/${record.variant_id}`} target="_blank" rel="noreferrer">{text}</a>
                        }
                        return text;
                      }
                    },
                    ...
                    },
                    {
                      ...
                      dataIndex: "variant_id",
                      render: (variant_id) => {
                        return <Button icon={<AiOutlineClose />} onClick={() => { handleDeleteVariant(variant_id) }} />
                      }
                    }
                  ]}
                  rowKey="id"
            ...
            <Item name={FormFiledStampPrinting.variants} rules={[
              {
                type: "array",
                required: true,
          ...
          <PickManyModal<PurchaseOrderLineItem>
            ...
            listItem={(item, index) => <ProductPOItem item={item} index={index} selectedIds={selectedIdsModal} setSelectedIds={setSelectedIdsModal} />}
        ...
        <BottomBarContainer
          ...
          rightComponent={<Button type="primary" onClick={() => form.submit()}>Xuất excel</Button>}
        />
