...
import { POProcumentField, POProcumentLineItemField, PurchaseProcumentLineItem } from "model/purchase-order/purchase-procument";
...
const ProcumentModal: React.FC<ProcumentModalProps> = (
  ...
  const { visible, now, stores, onCancle, items } = props;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Form.Item shouldUpdate={(prev, current) => prev[POProcumentField.line_items] !== current[POProcumentField.line_items]} noStyle>
            {({getFieldValue}) => {
              let line_items = getFieldValue(POProcumentField.line_items) ? getFieldValue(POProcumentField.line_items) : [];
              return (
                <Table
