...
const ProducmentInventoryMultiModal: React.FC<ProcumentInventoryModalProps> = (
  ...
  const { visible, title, onCancel, onOk, listProcurement, poData } = props;
  ...
  const onQuantityChange = useCallback(
    (quantity, index: number, item: PurchaseProcumentLineItem) => {
      ...
      cusData.forEach((e) => {
        if (e.sku === item.sku) e.real_quantity = quantity;
      });
  ...
  const getProcurementItems = useCallback(async () => {
    if (listProcurement && listProcurement.length > 0) {
      ...
      const res: PurchaseProcument = await callApiNative(
        { isShowLoading: false },
        dispatch,
        getProcurementsMerge,
        arrId.toString(),
      );
      if (res) {
  ...
  const exportExcel = useCallback(() => {
    ...
    const fileName = poData?.code
      ? `nhap_so_luong_phieu_nhap_kho_${poData?.code}.xlsx`
      : "nhap_so_luong_phieu_nhap_kho.xlsx";
      ...
      async (e: any) => {
        ...
        dataTable.forEach((e: PurchaseProcumentLineItem) => {
          const findItem = jsonData.find(
            (item: any) => item.sku !== undefined && item.sku.toString() === e.sku.toString(),
          );
      ...
      >
        <Row gutter={50}>
              ...
              value={
                listProcurement
                  ? ConvertUtcToLocalDate(
                      listProcurement[0].expect_receipt_date ?? "",
                      DATE_FORMAT.DDMMYYY,
                    )
                  : ""
              }
          ...
          <Col span={12}>
            <RowDetail
              title="Kho nhận hàng"
              value={listProcurement ? listProcurement[0].store ?? "" : ""}
            ></RowDetail>
          </Col>
        </Row>
        <Row gutter={50}>
          <Col span={12}>
            <RowDetail
              title="Nhà cung cấp"
              value={listProcurement ? listProcurement[0].purchase_order.supplier : ""}
            ></RowDetail>
          </Col>
          <Col span={12}>
            <div className="row-detail">
              ...
              <div className="row-detail-right data">
                {listProcurement ? listProcurement[0].purchase_order.phone : ""}
              </div>
            </div>
          </Col>
        </Row>
        <Table
          ...
          columns={[
            ...
            },
            {
              ...
              render: (value, item, index) => <div style={{ textAlign: "right" }}>{value}</div>,
            },
            {
              title: (
                <div
