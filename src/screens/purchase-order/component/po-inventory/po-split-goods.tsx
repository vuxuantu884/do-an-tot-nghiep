  ...
  useEffect(() => {
    if (dateOption) {
      const index = purchaseOrder?.procurements.findIndex(
        (procurement) =>
          moment(procurement.expect_receipt_date).format("DD/MM/YYYY") ===
          dateOption,
      );
      if (index >= 0) {
        const dataSource: PurchaseProcumentLineItem[] =
          purchaseOrder?.procurements[index].procurement_items.map(
            (procurementItem) => {
              const procurementsFilter = groupBy(
                purchaseOrder?.procurements,
                ProcurementLineItemField.expect_receipt_date,
              ...
              const acceptedQuantity = procurementsFilter[dateOption]
                ...
                .reduce(
                  (total, element) => total + element?.accepted_quantity || 0,
                  0,
                );
        ...
        <Col flex={1} style={{ textAlign: "end" }}>
          ...
          >
            {dateSelected.map((value) => {
              const dateString = moment(value.expect_receipt_date).format(
                "DD/MM/YYYY",
              );
              return (
                <Select.Option
                ...
                >
                  <StyledDate isChoose={dateString === dateOption}>
                    {dateString}
                  </StyledDate>
                  {value.status === ProcumentStatus.PARTIAL_RECEIVED && (
                    <StyledReceived
                      status={ProcumentStatus.PARTIAL_RECEIVED}
                      className="hidden"
                    >
                      Đã duyệt
                    </StyledReceived>
...
const StyledReceived = styled.span<{ status: string }>`
  ...
  margin-left: 4px;
  background: ${(p) =>
    p.status === ProcumentStatus.PARTIAL_RECEIVED ? "#27AE60" : "initial"};
  background: ${(p) =>
    p.status === ProcumentStatus.RECEIVED ? "#FCAF17" : "initial"};
  border-radius: 2px;
  color: #fff;
`;
