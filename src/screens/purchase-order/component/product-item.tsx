        ...
        <div className="product-item-info-right">
          {isTransfer ? (
            <span className="product-item-price">
              {price_data && price_data.retail_price ? formatCurrency(price_data.retail_price) : 0}{" "}
              <span className="currency">₫</span>
            </span>
          ) : (
            <span className="product-item-price">
              {price_data && price_data.import_price ? formatCurrency(price_data.import_price) : 0}{" "}
              <span className="currency">₫</span>
            </span>
          )}
          <span className="product-item-inventory">
            Số lượng tồn: <span className="value">{data.on_hand ? data.on_hand : 0}</span>
          </span>
        </div>
      </div>
    </div>
