    ...
    >
      ...
      >
        <Form.Item noStyle shouldUpdate={(prev, current) => prev.status !== current.status}>
          {({ getFieldValue }) => {
            if (isEditDetail) {
              return <DetailInfoPo formMain={formMain} />;
            }
  ...
  return (
    <Fragment>
      ...
      <PORowDetail
        title="Merchandiser"
        value={`${
          purchaseOrder.merchandiser_code && purchaseOrder.merchandiser_code.toUpperCase()
        } - ${purchaseOrder.merchandiser}`}
      />
      <PORowDetail
        title="QC"
        value={`${
          purchaseOrder.qc && purchaseOrder.qc_code
            ? `${purchaseOrder.qc_code.toUpperCase()} - ${purchaseOrder.qc}`
            : ""
        }`}
      />
      ...
      <PORowDetail
        title="Thiết kế"
        value={`${
          purchaseOrder.designer_code && purchaseOrder.designer
            ? `${purchaseOrder.designer_code.toUpperCase()} - ${purchaseOrder.designer}`
            : ""
        }`}
      />
