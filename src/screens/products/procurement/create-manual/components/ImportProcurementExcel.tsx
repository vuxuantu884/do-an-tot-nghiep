...
const ImportProcurementExcel: React.FC<ModalImportProps> = (
    ...
    onChange: useCallback(async (e: any) => {
      ...
      if (jsonData && data && data.length > 0) {
        ...
        for (let i = 0; i < jsonData.length; i++) {
          ...
          const findIndex = convertData.findIndex(e => e.sku && (e.sku.toString().trim() === element.sku.toString().trim()));
          if (findIndex >= 0) {
        ...
        if (convertData && convertData.length > 0) {
          for (let i = 0; i < convertData.length; i++) {
            ...
            const fi = data?.findIndex((e: PurchaseProcumentLineItemManual) => e.sku.toString().trim() === element.sku.toString().trim());

            if (element.quantity && (typeof element.quantity !== "number" || element.quantity % 1 !== 0)) {
