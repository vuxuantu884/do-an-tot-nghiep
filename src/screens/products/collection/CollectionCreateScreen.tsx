import { Space, Button, Card, Col, Form, FormInstance, Input, Row, Modal } from "antd";
...
const initialRequest: CollectionCreateRequest = {
  code: "",
  name: "",
};
...
const AddCollection: React.FC = () => {
  ...
  const footerConfirm = useMemo(() => {
    return (
      <>
        ...
        <Button key="create" type="default" onClick={onResetForm}>
          Tạo bst, nhóm khác
        </Button>
        <Button
          key="add"
  ...
  return (
    ...
    >
      <Form ref={formRef} onFinish={createCollection} initialValues={initialRequest} layout="vertical">
        <Card>
          <Row gutter={50}>
            <Col span={12}>
              <Form.Item
          ...
          <div className="modal-confirm-container">
            ...
            <div className="modal-confirm-right margin-left-20">
              ...
              <div className="modal-confirm-sub-title">
                Bạn có muốn thêm sản phẩm vào nhóm hàng này không?
              </div>
            </div>
          </div>
        </Modal>
