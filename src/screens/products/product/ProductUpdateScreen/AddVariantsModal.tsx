...
import { VariantRequestView, VariantResponse, ProductParams } from "model/product/product.model";
...
import { formatCurrencyForProduct, ArrDefects } from "screens/products/helper";
...
function AddVariantsModal(props: Props) {
  ...
  const cancelModal = (event: React.MouseEvent<HTMLElement, MouseEvent>) => {
    setIsVisible(false);
    onCancel?.(event);
  };

  const openModalAddVersion = () => {
    form.setFieldsValue({
      [modalInputField]: {
        color_id: [],
        size_id: [],
  ...
  const removeDuplicateVariant = () => {
    const variantList: VariantRequestView[] = variantTempList.filter(
      (variant) => typeof variant.color_id !== "number" || typeof variant.size_id === "number",
    );
  ...
  const handleCombineVariants = (
    colorIdList: number[],
    sizeIdList: number[],
    colorSelectedList: ColorResponse[],
    sizeSelectedList: SizeResponse[],
  ) => {
    ...
    if (variantName && variantCode) {
      ...
      setVariantTempList((prev) => {
        return uniqBy([...prev, ...newVariants], "sku");
      });
    }
  };
  ...
  const changeSize = (values: number[]) => {
  ...
  const changeColor = (values: number[]) => {
    if (Array.isArray(values) && values.length === 0) {
      return;
    }
            ...
            <Col md={4}>
              <Item
                ...
                tooltip={{
                  title: (
                    <div>
                      <b>Giá bán lẻ</b> là giá mà bạn sẽ bán sản phẩm này cho những khách hàng đơn
                      lẻ..
                    </div>
                  ),
                  icon: <InfoCircleOutlined />,
            ...
            <Col md={4}>
              <Item
                ...
                tooltip={{
                  title: () => (
                    <div>
                      <b>Giá buôn</b> là giá mà bạn sẽ bán sản phẩm này cho những khách hàng mua
            ...
            <Col md={4}>
              <Item
                ...
                tooltip={{
                  title: () => (
                    <div>
                      <b>Giá nhập</b> là giá mà nhập sản phẩm từ đơn mua hàng của nhà cung cấp.
                    </div>
                  ),
                  icon: <InfoCircleOutlined />,
                }}
            ...
            <Col md={4}>
              <Item
                ...
                tooltip={{
                  title: () => (
                    <div>
                      <b>Giá vốn</b> là tổng của những loại chi phí để đưa hàng có mặt tại kho.
                      Chúng bao gồm giá mua của nhà cung cấp, thuế giá trị gia tăng, chi phí vận
                      chuyển, bảo hiểm,...
                    </div>
                  ),
                  icon: <InfoCircleOutlined />,
          ...
          <Row gutter={20}>
            <Col md={12}>
              <Item name={[modalInputField, "color_id"]} label="Màu sắc">
                <BaseSelectPaging<ColorResponse>
                  mode="multiple"
