  ...
  const goDocument = useCallback((inventoryStatus: string,sku: string,variantName: string,store_id?:number)=>{
    ...
    switch (inventoryStatus) {
      ...
      case EInventoryStatus.IN_COMING:
        linkDocument =`${UrlConfig.PROCUREMENT}/products?page=1&limit=30${store_ids ? `&stores=${store_ids}`: ''}&content=${sku}`;
        break;
      case EInventoryStatus.ON_HOLD:
        linkDocument =`${UrlConfig.INVENTORY_TRANSFERS}?page=1&limit=30&simple=true${store_ids ? `&from_store_id=${store_ids}`: ''}&condition=${sku}&status=confirmed`;
        break;
      case EInventoryStatus.ON_WAY:
        linkDocument =`${UrlConfig.INVENTORY_TRANSFERS}?page=1&limit=30&simple=true${store_ids ? `&from_store_id=${store_ids}`: ''}&condition=${sku}&status=transferring`;
        break;
      case EInventoryStatus.TRANSFERRING:
        linkDocument =`${UrlConfig.INVENTORY_TRANSFERS}?page=1&limit=30&simple=true${store_ids ? `&to_store_id=${store_ids}`: ''}&condition=${sku}&status=transferring`;
        break;
      default:
        break;
    }
