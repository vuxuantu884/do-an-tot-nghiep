...
import { LogsStatus } from "screens/procurement/helper";
...
const TabLogs: React.FC = () => {
  ...
  const [isShowLogPo, setIsShowLogPo] = useState(false);
  const history = useHistory();

  const query = useQuery();
  let dataQuery: ProcumentLogQuery = { ...getQueryParams(query) };
  const [params, setParams] = useState<ProcumentLogQuery>(dataQuery);
  const [actionId, setActionId] = useState<number>();
  const [isVisibleProcurement, setIsVisibleProcurement] = useState(false);
    ...
    <TabLogsWrapper>
      ...
      {isShowLogPo && (
        <ActionPurchaseORderHistoryModal
          isModalVisible={isShowLogPo}
          onCancel={() => {
            setIsShowLogPo(false);
          }}
          actionId={actionId}
        />
      )}
      {isVisibleProcurement && (
