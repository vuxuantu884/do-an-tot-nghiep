...
function useFetchStoresKDOfflineTotalSales() {
    ...
    (data: any, asmData: any, columnKey: string) => {
      Object.keys(asmData).forEach((keyDriver: any) => {
        ...
        if (data.key === keyDriver && asmName) {
          ...
          if (columnKey === 'accumulatedMonth' && ![KeyDriverField.AverageOrderValue, KeyDriverField.AverageCustomerSpent].includes(keyDriver)) {
            data[`${asmName}_targetMonth`] = calculateTargetMonth(data[`${asmName}_accumulatedMonth`]);
          }
        } else {
          if (data.children?.length && [KeyDriverField.TotalSales, KeyDriverField.OfflineTotalSales].includes(data.key)) {
  ...
  const refetchStoresKDOfflineTotalSales = useCallback(() => {
    const fetchStoresKDOfflineTotalSales = async () => {
      ...
      const monthApi = (moment().date() > 1) ? callApiNative({ notifyAction: "SHOW_ALL" }, dispatch, getKDOfflineTotalSales, {
        from: START_OF_MONTH,
        to: YESTERDAY,
        posLocationNames: selectedStores,
        departmentLv2s: selectedAsm
