...
function Analytics() {
  ...
  const fetchCustomAnalytics = useCallback(async () => {
    ...
    if (onlyMe === undefined) {
      formFilterCustomReport.setFieldsValue({
        [FormFilterCustomReport.OnlyMyReport]: true,
      });
    }
    ...
    const response = await callApiNative(
      ...
      dispatch,
      getAnalyticsCustomByService,
      { "cube.in": cubes, onlyMe: onlyMe ?? true, "name.contains": nameReport },
    );
    if (response) {
  ...
  return (
    <ContentContainer
      ...
      breadcrumb={[
        { name: "Báo cáo" },
        { name: `Danh sách ${REPORT_NAMES[matchPath].toLocaleLowerCase()}` },
      ]}
    >
      <Form.Provider onFormFinish={handleFormFinish}>
        <ListAnalyticsStyle>
          ...
          <Card
            title="Báo cáo tuỳ chỉnh"
            className="card-custom-report"
            extra={
              <div>
                <Form className="btn-group" form={formFilterCustomReport}>
                  <Form.Item
                    className="m-0"
                    name={FormFilterCustomReport.OnlyMyReport}
                  ...
                  render: (id, item) => (
                    <div className="ana-list__action">
                      ...
                      currentUsername.toLocaleLowerCase() ===
                        item.created_by.toLocaleLowerCase() ? (
                        <>
                          <Button
                            icon={<AiOutlineEdit />}
                            onClick={(e: React.MouseEvent<HTMLElement>) =>
                              handleEditName(e, item.id, item.name)
                            }
                          ></Button>
                          <Button
                            icon={<FaTrashAlt color={Color.red} />}
        ...
        <ModalFormAnalyticsInfo
          ...
          handleOk={handleOkModalEditName}
          handleCancel={handleCancel}
        />
        <ModalCreateReport
          form={formCreateReport}
          isVisiable={isModalCreateVisible}
          handleOk={handleOkModalCreate}
