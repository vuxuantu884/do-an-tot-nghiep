...
function ModalCreateReport({ form, isVisiable, handleOk, handleCancel }: Props) {
  ...
  const onFinish = async (values: any) => {
    ...
    const reportGroup = Object.keys(ANALYTIC_TEMPLATE_GROUP)
    ...
    const { cube, query, timeAtOption, chart_query } =
      template as AnalyticCustomizeTemplateForCreate;
    if (cube && query) {
  ...
  const onChangeGroup = () => {
    setTemplateList(
      CUSTOMIZE_TEMPLATE[matchPath].filter(
        (item) =>
          form.getFieldValue("templateGroupIndex") === AnalyticCube.All ||
          form.getFieldValue("templateGroupIndex") === item.cube,
      ),
    );
  };
  ...
  useEffect(() => {
    setTemplateList(
      CUSTOMIZE_TEMPLATE[matchPath].filter(
        (item) =>
          ANALYTIC_TEMPLATE_GROUP[matchPath][0].cube === AnalyticCube.All ||
          ANALYTIC_TEMPLATE_GROUP[matchPath][0].cube === item.cube,
      ),
    );
  }, [matchPath]);
  return (
    ...
    >
      ...
      >
        ...
        <Form.Item
          label="Nhóm chỉ số"
          name="templateGroupIndex"
          rules={[{ required: true, message: "Vui lòng chọn nhóm chỉ số" }]}
        >
