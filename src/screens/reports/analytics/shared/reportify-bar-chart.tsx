...
function ReportifyBarChart(props: Props) {
  const { metadata, rowsInQuery, loadingChart } = useContext(AnalyticsContext);
  const {
    data,
    leftLegendName,
    rightLegendName,
        ...
        {!loadingChart ? (
          ...
          >
            ...
            {chartColumnNumber > 1 && (
              <Bar
                yAxisId="left"
                dataKey={(value) => value[value.length - 2]}
                fill={primary}
                name={leftLegendName}
                unit={leftTickFormat}
              />
            ...
            <Bar
              yAxisId={chartColumnNumber > 1 ? "right" : "left"}
              dataKey={(value) => value[value.length - 1]}
              fill={chartColumnNumber > 1 ? secondary : primary}
              name={rightLegendName}
              unit={rightTickFormat}
            />
          </BarChart>
        ) : (
          <div
        ...
        <div className="monthly-chart__tooltip">
          ...
          {payload.map((item: any, index: number) => (
            <span style={{ color: item?.color || "black" }} key={index}>
              ...
              {item.unit === FIELD_FORMAT.Price
                ? `${formatCurrency(parseFloat(item.value).toFixed(2))}₫`
                : item.value}
              <br />
            </span>
          ))}
