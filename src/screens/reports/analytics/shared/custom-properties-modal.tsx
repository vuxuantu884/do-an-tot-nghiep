...
function CustomPropertiesModal({ properties, aggregates, form, warningChooseColumn }: Props) {
  ...
  const handleChangeGroupCheckbox = (
    ...
    if (checked) {
      if (Array.isArray(name)) {
        ...
        // set rows in query
        setRowsInQuery([
          ...rowsInQuery.filter((item) => !allGroupKeys.includes(item)),
          ...allGroupKeys,
        ]);
        ...
        setActivePanel((prevState) => [...prevState, name[1]]);
  ...
  const handleOnChangeCheckboxProperties = (
    e: CheckboxChangeEvent,
    name: string,
    value: string,
  ) => {
  ...
  useEffect(() => {
    if (metadata && dataQuery) {
      ...
      const checkedKey =
        dataQuery?.query.columns.filter((item) => aggreateKeys.includes(item.field)) || [];
      setChosenProperties((prevState) => {
        return {
          ...prevState,
          [ReportifyFormFields.column]: checkedKey.map((item) => item.field),
        };
      });
      ...
      Object.keys(metadata?.properties).forEach((propertyKey) => {
        ...
        const checkedPropertyKey = dataQuery?.query.rows?.filter((item) =>
          propertyKeys.includes(item),
        );
        if (Array.isArray(checkedPropertyKey) && checkedPropertyKey.length > 0) {
          setChosenProperties((prevState) => {
            return {
              ...prevState,
              [propertyKey]: checkedPropertyKey.map((item) => item),
            };
          });

          setActivePanel((prevState) => [...prevState, propertyKey]);
        }
      });
    }
  }, [metadata, dataQuery]);
  return (
    <>
      <Button
        ...
        onClick={() => setVisiblePropertiesModal(true)}
        danger={warningChooseColumn}
        type="primary"
        ghost
      >
          ...
          <RoleStyled>
            ...
            >
              ...
              <Panel
                ...
                header={
                  <div className="panel-header">
                    <Checkbox
                      onChange={(e) =>
                        handleChangeGroupCheckbox(
                          e,
                          ReportifyFormFields.column,
                    ...
                    >
                      <b onClick={(e) => e.stopPropagation()}>
                        {_.capitalize("Thống kê")}
                        <span style={{ color: "red" }}> *</span>
                      </b>
                    </Checkbox>
                    <IconCheckboxCollapse
                      checked={activePanel.includes(ReportifyFormFields.column)}
                    />
                  </div>
                }
              >
                ...
                >
                  <Checkbox.Group className="panel-content">
                    {Object.keys(aggregates).map((item: any) => {
                      const value: any =
                        Object.values(aggregates)[Object.keys(aggregates).indexOf(item)].name;
                      return (
                        <Checkbox
                          key={item}
                          value={item}
                          ...
                          onChange={(e) =>
                            handleOnChangeCheckboxProperties(e, ReportifyFormFields.column, item)
                          }
                        >
                          {value}
                        </Checkbox>
              ...
              {Object.keys(properties).map((group: any) => {
                ...
                // kiểm tra nhóm properties đang ở trang thái checked hay indeterminate
                const isIndeterminate = !!checkArrayHasAnyValue(
                  chosenProperties[group],
                  groupPropertiesKeys,
                );
                const isChecked = checkArrayHasAllValue(
                  groupPropertiesKeys,
                  chosenProperties[group],
                );
                return (
                    ...
                    <Form.Item name={["properties", group]}>
                      <Checkbox.Group className="panel-content">
                        {Object.keys(groupProperties).map((item: any) => {
                          const value: any =
                            Object.values(groupProperties)[
                              Object.keys(groupProperties).indexOf(item)
                            ];
                          return (
                            <Checkbox
                              key={item}
                              value={item}
                              className="panel-content-item"
                              onChange={(e) => handleOnChangeCheckboxProperties(e, group, item)}
                            >
                              {value}
                            </Checkbox>
                          );
