...
function AnalyticsForm({ form, handleRQuery, mode, chartInfo }: Props) {
  ...
  const exportReportQuery = async (values: any) => {
    ...
    const columns = values[ReportifyFormFields.column]?.map((item: any) => ({
      field: item,
    }));
    ...
    if (timeGroupBy) {
      ...
      setRowsInQuery((prev: string[]) => [
        ...
        ...prev.filter(
          (item: string) =>
            TIME_GROUP_BY.findIndex((timeGroupItem) => timeGroupItem.value === item) === -1,
        ),
      ]);
    } else {
      setRowsInQuery((prev: string[]) => [
    ...
    if (orderBy?.length) {
      ...
      if (
        [...(values[ReportifyFormFields.column] || []), ...show].findIndex((item) =>
          orderByArr.includes(item),
        ) !== -1
      ) {
        isOrderBy = true;
      }
    }
    const rowsInQueryNoTimeGroup = rowsInQuery.filter(
      (item: string) =>
        TIME_GROUP_BY.findIndex((timeGroupItem) => timeGroupItem.value === item) === -1,
    );
    ...
    if (
      ...
      Object.keys(where).includes(ReportProperty.PosLocationName) &&
      where[ReportProperty.PosLocationName]
    ) {
  ...
  const handleFinishFailed = (errorInfo: any) => {
    ...
    if (Array.isArray(errorFields) && errorFields?.length > 0) {
      const isNotChooseColumn = errorFields.some((field) =>
        _.isEqual(field.name, [ReportifyFormFields.column]),
      );
    ...
    if ([YEAR, MONTH, DAY].includes(field)) {
      ...
      if (metadata) {
        Object.keys(metadata.properties).forEach((property) => {
          const keyIdx = Object.keys(metadata.properties[property]).findIndex(
            (propertyKey) => propertyKey === field,
          );
          if (keyIdx !== -1 && Object.keys(metadata.properties[property])[keyIdx + 1]) {
            ...
            setRowsInQuery((prev: string[]) =>
              nextRow
                ? [...prev.filter((item) => item !== currentRow && item !== nextRow), nextRow]
                : [...prev.filter((item) => item !== currentRow)],
            );
            fieldsValue.properties[property] = fieldsValue.properties[property].filter(
              (item: string) => item !== currentRow && item !== nextRow,
            );
            form.setFieldsValue({
              properties: {
                ...fieldsValue.properties,
                [property]: nextRow
                  ? [...fieldsValue.properties[property], nextRow]
                  : fieldsValue.properties[property],
              },
            });
          }
        });
      ...
      form.setFieldsValue({
        where: { ...fieldsValue.where, [field]: [selectedData] },
      });
    }
    form.submit();
    if (field === YEAR) {
  ...
  const handleRemoveFilter = (filter: any) => {
    ...
    switch (fieldFilter) {
      case MONTH:
        const timeFilters = Array.from(activeFilters.keys()).filter((field) =>
          [YEAR, DAY].includes(field as TIME),
        );
        switch (timeFilters.length) {
          ...
          case 1:
            timeFilter = {
              field: timeFilters[0],
              ...activeFilters.get(timeFilters[0]),
            };
            break;
          default:
        ...
        break;
      case DAY:
        const timeFilters2 = Array.from(activeFilters.keys()).filter((field) =>
          [YEAR, MONTH].includes(field as TIME),
        );
        switch (timeFilters2.length) {
          ...
          case 1:
            timeFilter = {
              field: timeFilters2[0],
              ...activeFilters.get(timeFilters2[0]),
            };
            break;
          default:
            timeFilter = undefined;
    ...
    if (!timeFilter && [YEAR, MONTH, DAY].includes(fieldFilter)) {
      switch (fieldFilter) {
        case YEAR:
          timeFilter = {
            field: YearNoChange,
            ...activeFilters.get(fieldFilter),
          };
          break;
        case MONTH:
          const currentYear = moment(
            activeFilters.get(fieldFilter).value[0],
            DATE_FORMAT.MMYYYY,
          ).year();
    ...
    if (timeFilter) {
      ...
      if (metadata) {
        Object.keys(metadata.properties).forEach((property) => {
          const keyIdx = Object.keys(metadata.properties[property]).findIndex(
            (propertyKey) => propertyKey === fieldFilter,
          );
          if (keyIdx !== -1) {
            const currentRow = Object.keys(metadata.properties[property])[keyIdx];
            ...
            form.setFieldsValue({
              properties: {
                ...fieldsValue.properties,
                [property]: [...(fieldsValue.properties[property] || []), currentRow],
              },
            });
          }
        });
      ...
      >
        <Card bodyStyle={{ paddingBottom: 8, paddingTop: 8 }} className="report-filter-wrapper">
          <div className="group-report-type">
            ...
            <Form.Item
              label=" "
              labelCol={{ span: 24, xs: 0, sm: 0 }}
              colon={false}
              className="filter-btn report-filter-item"
            >
        ...
        {permissionStores.length > 0 && (
          <Card bodyStyle={{ paddingBottom: 8, paddingTop: 8 }}>
              ...
              <Form.Item name={ReportifyFormFields.chartType} help={false} noStyle>
                ...
                >
                  {chartTypes.map((chartTypeItem: any) => {
                    return (
                      <Select.Option value={chartTypeItem.value} key={chartTypeItem.value}>
                        {chartTypeItem.label}
                      </Select.Option>
                    );
                  })}
              ...
              <Form.Item name={ReportifyFormFields.chartFilter} help={false} noStyle>
                {metadata && (
                  <Select
                    ...
                    onChange={_.debounce(
                      (value: [string]) => setChartColumnSelected(value),
                      AppConfig.TYPING_TIME_REQUEST,
                    )}
                    filterOption={(input, option) => {
                      if (option?.props.children) {
                        return strForSearch(option.props.children.toLowerCase()).includes(
                          strForSearch(input.toLowerCase()),
                        );
                      }
                      return false;
                    }}
        ...
        >
          {chartDataQuery &&
            ...
            chartType === ChartTypeValue.VerticalColumn && (
              <ReportifyBarChart
          ...
          {chartDataQuery &&
            (!chartInfo || chartInfo.showChart) &&
            chartType === ChartTypeValue.Pie && (
              <ReportifyPieChart
                data={chartDataQuery.result.data.reduce(
                  (res, chartDataItem) => {
                    const piePartName = chartDataItem
                      .filter((item: string, index) => index < rowsInQuery.length)
                      .reduce((res: string, item: string) => {
                        if (item) {
                    ...
                    const pieLegend1 = metadata
                      ...
                      : undefined;
                    const pieLegend2 =
                      metadata && chartColumnSelected?.length === 2
                        ? getTranslatePropertyKey(
                            metadata,
                            chartDataQuery.result.columns[chartDataQuery.result.columns.length - 2]
                              .field,
          ...
          {chartInfo && !chartInfo.showChart && <em>{chartInfo.message}</em>}
        </Card>
                ...
                return (
                  field !== "net_payments" && (
                    <Table.Column<any>
                      ...
                      sortOrder={
                        form.getFieldValue(ReportifyFormFields.orderBy) &&
                        form
                          .getFieldValue(ReportifyFormFields.orderBy)
                          .find((item: any[]) => item[0]?.toLowerCase() === field)
                          ? form
                              .getFieldValue(ReportifyFormFields.orderBy)
                              .find((item: any[]) => item[0]?.toLowerCase() === field)[1]
                              ?.toLowerCase() === "desc"
                            ? "descend"
                            : "ascend"
                          : undefined
                      ...
                      fixed={
                        index === 0 &&
                        format !== FIELD_FORMAT.Price &&
                        format !== FIELD_FORMAT.NumberFormat
                          ? "left"
                          : undefined
                      }
                      ...
                      render={(value, record: Array<any>) => {
                        ...
                        if (
                          ...
                          field !== TIME.HOUR &&
                          (!existedFilter ||
                            existedFilter.value.length > 1 ||
                            (existedFilter.value.length === 1 &&
                              existedFilter.value[0] === "Tất cả"))
                        ) {
                          return (
                            <span>
                              <span
                                className="link"
                                onClick={() =>
                                  handleQueryColumn(item, format === "price" ? record[index] : data)
                                }
                              >
                                {data}
                              </span>
                              {detailLink ? (
                                <Link
                                  ...
                                  to={`${detailLink.link}/${formatDataToSetUrl(
                                    data,
                                    detailLink.field,
                                  )}`}
                                >
                                  <div className="external-link">
                                    <img
                                      src={
                                        require(`assets/icon/feather-arrow-down-right.svg`).default
                                      }
                                      alt={"Xem chi tiết"}
                                    />
                                  </div>
