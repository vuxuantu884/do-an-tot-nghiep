    ...
    <RangePicker
      ...
      onOpenChange={(open) => {
        if (!stateTimeList.length) {
          const selectedIdx = FIXED_DATE_LIST.findIndex((timeItem) =>
            compare2RangeDate(timeItem.value, value),
          );
          if (selectedIdx !== -1) {
          ...
          const selectedGroup = FIXED_DATE_LIST.filter((timeItem) =>
            compare2RangeDate(timeItem.value, value),
          );
          if (selectedGroup.length) {
            const selectedIdx = stateTimeList.findIndex(
              (item) =>
                item.selected &&
                selectedGroup.findIndex((timeItem) => timeItem.label === item.label) !== -1,
            );
            if (selectedIdx === -1) {
              setStateTimeList([
                ...FIXED_DATE_LIST.map((item) => ({
                  ...item,
                  selected: item.label === selectedGroup[0].label ? true : false,
                })),
              ]);
            }
          } else {
            ...
            <AnalyticsDatePickerStyle>
              <div className="picker-panel">
                {stateTimeList.map((item, index) => {
                  return (
                    <Button
                      className={`picker-panel__item  ${item.selected ? "ant-btn-primary" : ""}`}
                      key={index}
                      onClick={() => {
                        onChange(item.value);
                        setOnOpen(false);
                        item.selected = true;
                        setStateTimeList((prevState) => [
                          ...prevState.map((dateItem) => {
