...
function CustomerVisitors() {
  ...
  useEffect(() => {
    const fetchCustomerVisitors = async () => {
      ...
      const customerVisitors = await callApiNative(
        { isShowError: true },
        dispatch,
        getCustomerVisitors,
        { month: monthQuery, year: yearQuery, storeIds },
      );
      if (customerVisitors) {
  ...
  const handleUpdateCustomerVisitors = async (storeId: any) => {
    const params =
      customerVisitorsUpdate &&
      customerVisitorsUpdate.find((item: any) => item.store_id === storeId);
    if (params) {
                        ...
                        return index > 0 ? (
                          index === columns.length - 1 ? (
                            <Button
                              ...
                              className="px-1"
                              size="small"
                              onClick={() => handleUpdateCustomerVisitors(record.store_id)}
                            >
                              Cập nhật
                            </Button>
                          ) : (
                            <div className={item.isToday ? "current-day" : ""}>
                              <Input
                                ...
                                onChange={(e) => handleChangeVisitors(record, item.key, e)}
                              />
                            </div>
                          )
                        ) : (
