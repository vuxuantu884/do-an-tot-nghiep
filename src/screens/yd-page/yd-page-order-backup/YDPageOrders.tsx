...
export default function YDPageOrders(props: any) {
  const {
    customer,
    setCustomer,
    setActiveTabKey,
      ...
      if (items.length === 0) {
        ...
        if (shipmentMethod === ShipmentMethodOption.SELF_DELIVER) {
          if (values.delivery_service_provider_id === null) {
            ...
            dispatch(
              orderYDPpageCreateAction(values, createOrderCallback, setLoadingCreateButton)
            );
          }
        } else {
          if (
  ...
  const checkInventory = () => {
    let status = true;
    if (inventoryResponse && inventoryResponse.length && items) {
