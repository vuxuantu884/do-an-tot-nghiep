...
function ReportPreparationShopeeProductModal(props: ReportPreparationShopeeProductModalProps) {
  ...
  return (
    <Modal
      ...
      okButtonProps={{
        disabled: showButtonConfirm && (!selectedRow.length || !params.ecommerce_shop_ids.length),
      }}
    >
      <Radio.Group onChange={onChangeBatchShippingOption} value={batchShippingType}>
        <Space direction="vertical">
          <Radio
            ...
            value={BATCHING_SHIPPING_TYPE.SELECTED}
            onClick={handleShowButtonConfirm}
          >
            Báo các đơn hàng đã chọn
          </Radio>
          <Radio
            disabled={isReportShopeeFilter || !params.ecommerce_shop_ids.length || total === 0}
            value={BATCHING_SHIPPING_TYPE.FILTERED}
            onClick={handleShowButtonConfirm}
          >
