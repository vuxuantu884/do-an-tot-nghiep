...
const EcommerceOrderFilter: React.FC<EcommerceOrderFilterProps> = (
  ...
  const onChangeTag = useCallback((value: any) => {
    setTags(value);
  }, []);
    ...
    (values) => {
      ...
      if (error) {
        ...
        values.issued_on_min = formatDateTimeOrderFilter(
          values.issued_on_min || initialValues.issued_on_min,
          dateFormat,
        );
        values.issued_on_max = formatDateTimeOrderFilter(
          values.issued_on_max || initialValues.issued_on_max,
          dateFormat,
        );
        values.finalized_on_min = formatDateTimeOrderFilter(
          values.finalized_on_min || initialValues.finalized_on_min,
          dateFormat,
        );
        values.finalized_on_max = formatDateTimeOrderFilter(
          values.finalized_on_max || initialValues.finalized_on_max,
          dateFormat,
        );
        values.cancelled_on_min = formatDateTimeOrderFilter(
          values.cancelled_on_min || initialValues.cancelled_on_min,
          dateFormat,
        );
        values.cancelled_on_max = formatDateTimeOrderFilter(
          values.cancelled_on_max || initialValues.cancelled_on_max,
          dateFormat,
        );
        values.completed_on_min = formatDateTimeOrderFilter(
          values.completed_on_min || initialValues.completed_on_min,
          dateFormat,
        );
        values.completed_on_max = formatDateTimeOrderFilter(
          values.completed_on_max || initialValues.completed_on_max,
          dateFormat,
        );
    ...
    if (key !== ecommerceKeySelected) {
      ...
      onFilter &&
        onFilter({
          ...params,
          ecommerce_shop_ids: [],
          channel_codes: ecommerceSelected?.key,
        });
    }
  };
  ...
  let filters = useMemo(() => {
    ...
    if (initialValues?.channel_codes?.length === 1) {
      ...
      initialValues.channel_codes.forEach((ecommerceCode: any) => {
        const ecommerceSelected = ECOMMERCE_LIST?.find(
          (ecommerce) => ecommerce.key?.toString() === ecommerceCode?.toString(),
        );
        ecommerceFilterText = ecommerceSelected
          ? ecommerceFilterText + ecommerceSelected.title + "; "
          : ecommerceFilterText;
      });
      list.push({
        key: "channel_codes",
    ...
    if (initialValues.ecommerce_shop_ids.length) {
      ...
      initialValues.ecommerce_shop_ids.forEach((shop_id: any) => {
        const selectedShop = ecommerceShopList?.find(
          (shop) => shop.id.toString() === shop_id.toString(),
        );
    ...
    if (initialValues.payment_method_ids.length) {
      ...
      initialValues.payment_method_ids.forEach((paymentId: any) => {
        const findStatus = listPaymentMethod?.find(
          (item) => item.id.toString() === paymentId.toString(),
        );
        textStatus = findStatus ? textStatus + findStatus.name + "; " : textStatus;
      });
      list.push({
    ...
    if (initialValues.delivery_provider_ids.length) {
      ...
      initialValues.delivery_provider_ids.forEach((deliveryId: any) => {
        const findType = deliveryService?.find(
          (item) => item.id.toString() === deliveryId.toString(),
        );
        textType = findType ? textType + findType.name + "; " : textType;
      });
      list.push({
  ...
  useEffect(() => {
    ...
    if (!params.tags.length) {
      ...
      let tagsArr: any[] = [];
      const tagsFilter = Array.isArray(params.tags) ? params.tags : [params.tags];
        ...
        <Form onFinish={onFinish} form={form} initialValues={initialValues}>
          <div className="order-filter-container">
            <div className="first-line">
              ...
              <Item className="ecommerce-dropdown" name="channel_codes">
                <Select
                  disabled={isLoading}
              ...
              <Form.Item className="select-store-dropdown" name="ecommerce_shop_ids">
                {isEcommerceSelected ? (
                  <TreeSelect
                ...
                <Col span={8} xxl={8}>
                  <Item name="return_status" label="Trả hàng">
                    <CustomSelect
                      ...
                      getPopupContainer={(trigger) => trigger.parentNode}
                    >
                      <CustomSelect.Option style={{ width: "100%" }} key="1" value="1">
