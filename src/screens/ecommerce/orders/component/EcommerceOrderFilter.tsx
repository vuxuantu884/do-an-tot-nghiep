...
const EcommerceOrderFilter: React.FC<EcommerceOrderFilterProps> = (
  ...
  const onFinish = useCallback(
    ...
    [ecommerceSelected, formRef, onFilter, shopIdSelected]
  );
            ...
            <div key={item.id} className="shop-name">
              <Checkbox onChange={(e) => onSelectShopChange(item, e)} checked={item.isSelected}>
                <span className="check-box-name">
                  ...
                  {item.name && item.name.length <= 31 && <span className="name">{item.name}</span>}
                </span>
              </Checkbox>
            </div>
          ))}
  ...
  const handleSelectEcommerce = (ecommerce_id: any) => {
    ...
    setEcommerceSelected(ecommerce_id);
  };
  ...
  useEffect(() => {
    ...
    if (!initialValues.ecommerce_shop_ids.length) {
      ...
      let ecommerce_shop_ids = Array.isArray(params.ecommerce_shop_ids)
        ? params.ecommerce_shop_ids
        : [params.ecommerce_shop_ids];
      ecommerce_shop_ids.forEach((x) => {
        itemSelected.push(Number(x));
        return ecommerceShopList.forEach((y) => {
          return itemSelected.includes(y.id) ? (y.isSelected = true) : (y.isSelected = false);
        });
      });
      setShopIdSelected(itemSelected);
    }
    ...
    <StyledOrderFilter>
      <div className="order-filter">
        <Form onFinish={onFinish} ref={formSearchRef} initialValues={initialValues}>
          ...
          <Item className="ecommerce-dropdown" name="ecommerce_id">
            <Select
              showSearch
              disabled={isLoading}
              ...
              {ECOMMERCE_LIST?.map((item: any) => (
                <Option key={item.ecommerce_id} value={item.ecommerce_id.toString()}>
                  <div>
          ...
          <Item>
            <Button icon={<FilterOutlined />} onClick={openFilter} disabled={isLoading}>
              Thêm bộ lọc
            </Button>
          </Item>
