...
function PreparationShopeeProductModal(props: PreparationShopeeProductModalProps) {
  ...
  const handleGetEcommerceShopAddress = (value: number) => {
    ...
    ecommerceShopAddress.forEach((item) => {
      ...
      const newListShopPickUpAddress = listShopPickUpAddress?.filter(
        (shopPick: any) => shopPick.shop_id !== item.shop_id,
      );

      const newListShopOrderList = listShopOrderList?.filter(
        (shopOrderList: any) => shopOrderList.shop_id !== item.shop_id,
      );
  ...
  useEffect(() => {
    ecommerceShopAddress.length &&
      ecommerceShopAddress.map(
        (item) => item.store_addresses === null && setCheckStoreAddress(true),
      );
  }, [ecommerceShopAddress]);
  ...
  return (
    ...
    >
      {ecommerceShopAddress.length &&
        ecommerceShopAddress?.map((shop) => (
          <div key={shop.address_id}>
            <div style={{ display: "flex", alignItems: "center" }}>
              <p style={{ margin: 0 }}>Địa chỉ lấy hàng:</p>
              <span style={{ fontWeight: 600, paddingLeft: 6 }}>
                {ecommerceShopListByAddress.map(
                  (list) => list.id === shop.shop_id && `Gian hàng ${list.name.toUpperCase()}`,
                )}
              </span>
            </div>
            {shop.store_addresses !== null ? (
              ...
              >
                {shop.store_addresses.map((shop: any) => (
                  <Option key={shop.address_id} value={shop.address_id}>
