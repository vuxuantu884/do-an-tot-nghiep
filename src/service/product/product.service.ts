...
export const productGetHistory = (
  query: ProductHistoryQuery,
): Promise<BaseResponse<PageResponse<ProductHistoryResponse>>> => {
  const queryString = generateQuery(query);
  return BaseAxios.get(`${ApiConfig.PRODUCT}/products/histories?${queryString}`);
};

export const productGetHistoryInTem = (
  query: ProductHistoryQuery,
): Promise<BaseResponse<PageResponse<ProductHistoryResponse>>> => {
  const queryString = generateQuery(query);
...
export const getJobByCode = (code: string): Promise<BaseResponse<JobResponse>> => {
...
export const getFile = (code: string): Promise<BaseResponse<JobResponse>> => {
