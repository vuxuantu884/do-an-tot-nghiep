...
import {
  ...
  ColorSearchQuery,
  ColorUpdateRequest,
} from "model/product/color.model";

export const colorSearchApi = (
  query?: ColorSearchQuery,
): Promise<BaseResponse<PageResponse<ColorResponse>>> => {
...
export const colorCreateApi = (
  request: ColorCreateRequest,
): Promise<BaseResponse<ColorResponse>> => {
  return BaseAxios.post(`${ApiConfig.PRODUCT}/colors`, request);
};

export const colorUpdateApi = (
  id: number,
