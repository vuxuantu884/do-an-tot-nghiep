...
const TreeStore = (props: Props) => {
  ...
  const propConvert = () => {
    ...
    const restPropsConvert = {
      ...restProps,
      value: Array.isArray(restPropsExt?.value)
        ? restPropsExt?.value?.map((p: string | number) => +p)
        : [restPropsExt?.value],
    };
  ...
  useEffect(() => {
    ...
    const grouped: any =
      listStore !== undefined ? groupBy(listStore, (store: StoreResponse) => store.department) : [];
