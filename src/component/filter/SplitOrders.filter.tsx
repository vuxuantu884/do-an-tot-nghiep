import { Button, Col, Form, FormInstance, Input, Row, Tag, InputNumber, Radio } from "antd";
...
const SplitOrdersFilter: React.FC<SplitOrdersFilterProps> = (props: SplitOrdersFilterProps) => {
  const {
    params,
    actions,
    listSource,
  ...
  let filters = useMemo(() => {
    ...
    if (initialValues.store_ids.length) {
      let mappedStores = listStore?.filter((store) =>
        initialValues.store_ids?.some((single) => single === store.id.toString()),
      );
