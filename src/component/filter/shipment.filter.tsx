import { Button, Col, Form, FormInstance, Input, Row, Select, Tag, Radio } from "antd";
...
const OrderFilter: React.FC<OrderFilterProps> = (props: OrderFilterProps) => {
  const {
    params,
    actions,
    listSource,
  ...
  const initialValues = useMemo(() => {
    return {
      ...params,
      stock_location_ids: Array.isArray(params.stock_location_ids)
        ? params.stock_location_ids
        : [params.stock_location_ids],
      source_ids: Array.isArray(params.source_ids) ? params.source_ids : [params.source_ids],
      reference_status: Array.isArray(params.reference_status)
        ? params.reference_status
        : [params.reference_status],
      ...
      delivery_types: Array.isArray(params.delivery_types)
        ...
        : [params.delivery_types],
      print_status: Array.isArray(params.print_status)
        ? params.print_status
        : [params.print_status],
      pushing_status: Array.isArray(params.pushing_status)
        ? params.pushing_status
        : [params.pushing_status],
  ...
  const changeStatusPrint = useCallback(
    (status) => {
  ...
  const changeStatusPushing = useCallback(
    (status) => {
  ...
  let filters = useMemo(() => {
    ...
    if (initialValues.stock_location_ids.length) {
      let mappedStores = listStore?.filter((store) =>
        initialValues.stock_location_ids?.some((single) => single === store.id.toString()),
      );
    ...
    if (initialValues.source_ids.length) {
      let mappedSources = listSources?.filter((source) =>
        initialValues.source_ids?.some((single) => single === source.id.toString()),
      );
    ...
    if (initialValues.packed_on_min || initialValues.packed_on_max) {
      let textOrderCreateDate =
        (initialValues.packed_on_min ? initialValues.packed_on_min : "??") +
        " ~ " +
        (initialValues.packed_on_max ? initialValues.packed_on_max : "??");
      list.push({
        key: "packed",
        name: "Ngày đóng gói",
    ...
    if (initialValues.ship_on_min || initialValues.ship_on_max) {
      let textOrderShipDate =
        (initialValues.ship_on_min ? initialValues.ship_on_min : "??") +
        " ~ " +
        (initialValues.ship_on_max ? initialValues.ship_on_max : "??");
      list.push({
        key: "ship",
        name: "Ngày giao hàng",
    ...
    if (initialValues.shipped_on_min || initialValues.shipped_on_max) {
      let textExpectReceiveDate =
        (initialValues.shipped_on_min ? initialValues.shipped_on_min : "??") +
        " ~ " +
        (initialValues.shipped_on_max ? initialValues.shipped_on_max : "??");
      list.push({
        key: "shipped",
    ...
    if (initialValues.reference_status.length) {
      let mappedSources = controlStatus?.filter((item) =>
        initialValues.reference_status?.some((single) => single === item.value),
      );
      let text = getFilterString(mappedSources, "name", undefined, undefined);
      list.push({
    ...
    if (initialValues.print_status.length) {
      let mappedPaymentMethods = printStatus?.filter((single) =>
        initialValues.print_status?.some((item) => item === single.value),
      );
    ...
    if (initialValues.pushing_status.length && !isPushingStatusFailed) {
      let mappedPaymentMethods = pushingStatus?.filter((single) =>
        initialValues.pushing_status?.some((item) => item === single.value),
      );
      let text = getFilterString(mappedPaymentMethods, "name", undefined, undefined);
      list.push({
        key: "pushing_status",
    ...
    if (initialValues.reason_ids.length) {
      let mappedPaymentMethods = reasons?.filter((single) =>
        initialValues.reason_ids?.some((item) => item === single.id.toString()),
      );
      let text = getFilterString(mappedPaymentMethods, "name", undefined, undefined);
      list.push({
        key: "reason_ids",
    ...
    initialValues.stock_location_ids,
    initialValues.source_ids,
    initialValues.packed_on_min,
    initialValues.packed_on_max,
    initialValues.ship_on_min,
    initialValues.ship_on_max,
    initialValues.exported_on_min,
    initialValues.exported_on_max,
    initialValues.cancelled_on_min,
    initialValues.cancelled_on_max,
    initialValues.shipped_on_min,
    initialValues.shipped_on_max,
    initialValues.reference_status,
    initialValues.shipper_codes,
    initialValues.delivery_provider_ids,
    initialValues.print_status,
    initialValues.pushing_status,
  ...
  useEffect(() => {
    ...
    setPushing(
      Array.isArray(params.pushing_status) ? params.pushing_status : [params.pushing_status],
    );
    setControl(
      Array.isArray(params.reference_status) ? params.reference_status : [params.reference_status],
    );
                ...
                <Col span={12} xxl={8}>
                  ...
                  <Item name="reason_ids">
                    <Select
                      mode="multiple"
                      showSearch
                      placeholder="Chọn lý do huỷ giao"
                      notFoundContent="Không tìm thấy kết quả"
                      style={{ width: "100%" }}
                      optionFilterProp="children"
                      showArrow
