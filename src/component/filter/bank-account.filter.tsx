...
const BankAccountFilter: React.FC<BankAccountType> = (props: BankAccountType) => {
  ...
  const onFinish = useCallback(
    (value) => {
      ...
      value.status = onFilter && onFilter(query);
    },
    [onFilter],
  );
  ...
  return (
    <React.Fragment>
      <Form layout="inline" ref={formRef} onFinish={onFinish} initialValues={initialValues}>
        <FilterWrapper>
          <Form.Item name="account_numbers" className="search" style={{ width: "100%" }}>
            <Input prefix={<img src={search} alt="" />} placeholder="Tìm kiếm theo số tài khoản" />
          </Form.Item>
          ...
          <Form.Item name="status" style={{ width: "75%" }}>
            <Select placeholder="Chọn trạng thái" allowClear showArrow>
