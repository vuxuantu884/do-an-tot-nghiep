...
const ProductWrapperFilter: React.FC<ProductFilterProps> = (
  ...
  const [createTimeOnMin, setCreateTimeOnMin] = useState(initialValues.from_create_date? moment(initialValues.from_create_date, "YYYY-MM-DD") : null);
  const [createTimeOnMax, setCreateTimeOnMax] = useState(initialValues.to_create_date? moment(initialValues.to_create_date, "YYYY-MM-DD") : null);
  ...
  const onFinish = useCallback(
    (values) => {
      const valuesForm = {
        ...values,
        from_create_date: createTimeOnMin ? moment(createTimeOnMin, 'YYYY-MM-DD')?.format('YYYY-MM-DD') : null,
        to_create_date: createTimeOnMax ? moment(createTimeOnMax, 'YYYY-MM-DD').format('YYYY-MM-DD') : null,
      }
  ...
  const clickOptionDate = useCallback(
    ...
    switch(value) {
      case 'today':
        ...
        break
      case 'yesterday':
        minValue = moment().startOf('day').subtract(1, 'days').format('YYYY-MM-DD')
        maxValue = moment().endOf('day').subtract(1, 'days').format('YYYY-MM-DD')
        break
      case 'thisweek':
        ...
        break
      case 'lastweek':
        minValue = moment().startOf('week').subtract(1, 'weeks').format('YYYY-MM-DD')
        maxValue = moment().endOf('week').subtract(1, 'weeks').format('YYYY-MM-DD')
        break
      case 'thismonth':
        ...
        break
      case 'lastmonth':
        minValue = moment().startOf('month').subtract(1, 'months').format('YYYY-MM-DD')
        maxValue = moment().endOf('month').subtract(1, 'months').format('YYYY-MM-DD')
        break  
      default:
        break
    }
    ...
    (e, tag) => {
      ...
      switch(tag.key) {
        case 'create_time':
          ...
          setCreateTimeOnMin(null)
          setCreateTimeOnMax(null)
          onFilter && onFilter({...params, from_create_date: undefined, to_create_date: undefined});
          break;
        default: break
      }
        ...
        >
          <PWFormFilter>
            ...
            <Row className="create-date" gutter={12}>
              <Col span={24}>
                <Collapse defaultActiveKey={initialValues.from_create_date && initialValues.to_create_date ? ["1"]: []}>
                  <Panel header="NGÀY KHỞI TẠO" key="1" className="header-filter">
                    <div className="date-option">
                      <Button onClick={() => clickOptionDate('create_time', 'yesterday')} className={createTimeClick === 'yesterday' ? 'active' : 'deactive'}>Hôm qua</Button>
                      <Button onClick={() => clickOptionDate('create_time', 'today')} className={createTimeClick === 'today' ? 'active' : 'deactive'}>Hôm nay</Button>
                      <Button onClick={() => clickOptionDate('create_time', 'thisweek')} className={createTimeClick === 'thisweek' ? 'active' : 'deactive'}>Tuần này</Button>
                    </div>
                    <div className="date-option">
                      <Button onClick={() => clickOptionDate('create_time', 'lastweek')} className={createTimeClick === 'lastweek' ? 'active' : 'deactive'}>Tuần trước</Button>
                      <Button onClick={() => clickOptionDate('create_time', 'thismonth')} className={createTimeClick === 'thismonth' ? 'active' : 'deactive'}>Tháng này</Button>
                      <Button onClick={() => clickOptionDate('create_time', 'lastmonth')} className={createTimeClick === 'lastmonth' ? 'active' : 'deactive'}>Tháng trước</Button>
                    </div>
                    <p><SettingOutlined style={{marginRight: "10px"}}/>Tuỳ chọn khoảng thời gian:</p>
                    <DatePicker.RangePicker
                      format="YYYY-MM-DD"
                      style={{width: "100%"}}
                      value={[createTimeOnMin? moment(createTimeOnMin, "YYYY-MM-DD") : null, createTimeOnMax ? moment(createTimeOnMax, "YYYY-MM-DD") : null]}
        ...
        {filters && filters.map((filter: any) => {
          return (
            <Tag className="tag" closable onClose={(e) => onCloseTag(e, filter)}>
              {
                filter.name ? `${filter.name}: ${filter.value}` : `${filter.value}`
              }
              </Tag>
          )
        })}
