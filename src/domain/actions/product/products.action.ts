...
export const productWrapperDeleteAction = (
  ids: Array<number>,
  onDeleteSuccess: (res: any) => void,
) => {
  return BaseAction(ProductType.DELETE_PRODUCT_WRAPPER_REQUEST, {
    ids,
    onDeleteSuccess,
  });
};
...
export const productUpdateAction = (
  id: number,
  request: ProductRequest | ProductResponse,
  onResult: (result: ProductResponse | false) => void,
) => {
  return BaseAction(ProductType.PRODUCT_UPDATE, { id, request, onResult });
};

export const productBarcodeAction = (
  request: ProductBarcodeRequest,
  onResult: (result: string | false) => void,
) => {
  return BaseAction(ProductType.PRODUCT_BARCODE, { request, onResult });
};

export const productImportAction = (
  file: File,
  isCreate: string,
  onResult: (result: string | false) => void,
) => {
...
export const variantUpdateManyAction = (
  ...
  return BaseAction(ProductType.VARIANT_UPDATE_SALEABLE, {
    variants,
    onResult,
  });
};

export const variantDeleteManyAction = (
  variants: Array<any>,
  onResult: (exception: boolean) => void,
) => {
...
export const productCheckDuplicateCodeAction = (
  code: string,
  onResult: (duplicateMessages: string) => void,
) => {
