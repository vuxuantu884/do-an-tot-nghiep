...
export const getCategoryRequestAction = (query: CategoryQuery, setData: (data: Array<CategoryResponse>) => void) => {
 ...
 export const createCategoryAction = (request: CategoryCreateRequest, onCreateSuccess: (result: CategoryResponse) => void) => {
  return BaseAction(CategoryType.CREATE_CATEGORY_REQUEST, {request, onCreateSuccess});
 }

export const categoryDetailAction = (id: number, setData: (data: false|CategoryResponse) => void) => {
  return BaseAction(CategoryType.DETAIL_CATEGORY_REQUEST, {id, setData});
}

export const categoryUpdateAction = (id: number, request: CategoryUpdateRequest, onUpdateSuccess: (result: CategoryResponse) => void) => {
