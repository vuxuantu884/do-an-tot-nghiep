...
export const LoyaltyCardSearch = (
  query: CustomerCardListRequest,
  setData: (data: PageResponse<LoyaltyCardResponse>) => void,
) => {
  return BaseAction(LoyaltyCardType.SEARCH_LOYALTY_CARD_REQUEST, {
    query,
    setData,
  });
};

export const LoyaltyCardAssignment = (
  id: number,
  query: LoyaltyCardAssignmentRequest,
  callback: (data: BaseResponse<LoyaltyCardResponse>) => void,
) => {
  return BaseAction(LoyaltyCardType.ASSIGN_CUSTOMER_REQUEST, {
    id,
    query,
    callback,
  });
