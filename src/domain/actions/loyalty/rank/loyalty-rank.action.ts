...
export const LoyaltyRankSearch = (query: LoyaltyRankSearchRequest, setData: (data: PageResponse<LoyaltyRankResponse>) => void) => {
    return BaseAction(LoyaltyRankType.SEARCH_LOYALTY_RANK_REQUEST, { query, setData });
}

export const CreateLoyaltyRank = (body: CreateLoyaltyRequest, callback: (data: LoyaltyRankResponse) => void) => {
    return BaseAction(LoyaltyRankType.CREATE_LOYALTY_RANK_REQUEST, { body, callback });
}

export const GetLoyaltyRankDetail = (id: number, callback: (data: LoyaltyRankResponse) => void) => {
    return BaseAction(LoyaltyRankType.GET_LOYALTY_RANK_DETAIL_REQUEST, { id, callback });
}
