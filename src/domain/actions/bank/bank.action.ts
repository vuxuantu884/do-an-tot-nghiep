...
export const getBankAccountAction = (
  query: BankAccountSearchQuery,
  setData: (data: PageResponse<BankAccountResponse>) => void,
) => {
  return BaseAction(BankType.GET_BANK_ACCOUNT, { query, setData });
};

export const postBankAccountAction = (
  param: BankAccountRequest,
  setData: (data: BankAccountResponse) => void,
) => {
  return BaseAction(BankType.POST_BANK_ACCOUNT, { param, setData });
};

export const putBankAccountAction = (
  id: number,
  param: BankAccountRequest,
  setData: (data: BankAccountResponse) => void,
) => {
  return BaseAction(BankType.PUT_BANK_ACCOUNT, { id, param, setData });
};

export const getIdBankAccountAction = (
  id: number,
  setData: (data: BankAccountResponse) => void,
) => {
