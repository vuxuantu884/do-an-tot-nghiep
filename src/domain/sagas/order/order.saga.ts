...
function* getListOrderCustomerSaga(action: YodyAction) {
  let { query, setData } = action.payload;
  try {
    let response: BaseResponse<Array<OrderModel>> = yield call(getListOrderCustomerApi, query);
    if (isFetchApiSuccessful(response)) {
      setData(response.data);
    } else {
...
function* updateFulFillmentStatusSaga(action: YodyAction) {
  ...
  try {
    let response: BaseResponse<OrderResponse> = yield call(updateFulFillmentStatus, request);
    if (isFetchApiSuccessful(response)) {
      setData(response.data);
    } else {
...
function* rePushFulFillmentSaga(action: YodyAction) {
  ...
  try {
    let response: BaseResponse<any> = yield call(rePushFulFillmentService, fulfillment_id);
    if (isFetchApiSuccessful(response)) {
      setData(response.data);
    } else {
...
function* updatePaymentSaga(action: YodyAction) {
  ...
  yield put(showLoading());
  try {
    let response: BaseResponse<OrderResponse> = yield call(updatePayment, request, order_id);
    if (isFetchApiSuccessful(response)) {
      setData(response.data);
    } else {
      setError(true);
...
function* PaymentMethodGetListSaga(action: YodyAction) {
  ...
  try {
    ...
    showError(
      "Có lỗi khi lấy dữ liệu danh sách cách thức thanh toán đơn hàng! Vui lòng thử lại sau!",
    );
  } finally {
    yield put(hideLoading());
  }
...
function* getFulfillmentDetailSaga(action: YodyAction) {
  ...
  yield put(showLoading());
  try {
    let response: BaseResponse<any> = yield call(getFulFillmentDetailAction, fulfillment_code);
    if (isFetchApiSuccessful(response)) {
      setData(response.data);
    } else {
...
function* getTRackingLogFulfillmentSaga(action: YodyAction) {
  ...
  try {
    ...
    showError(
      "Có lỗi khi lấy dữ liệu danh sách bản ghi trạng thái đơn hàng! Vui lòng thử lại sau!",
    );
  } finally {
    yield put(hideLoading());
  }
...
function* getTRackingLogErrorSaga(action: YodyAction) {
  ...
  try {
    ...
    showError(
      "Có lỗi khi lấy dữ liệu danh sách bản ghi lỗi trạng thái đơn hàng! Vui lòng thử lại sau!",
    );
  }
}
...
function* getListSubStatusSaga(action: YodyAction) {
  ...
  try {
    let response: BaseResponse<PageResponse<Array<OrderSubStatusResponse>>> = yield call(
      getOrderSubStatusService,
      status,
    );
    if (isFetchApiSuccessful(response)) {
...
function* getListReasonSaga(action: YodyAction) {
  const { setData } = action.payload;
  try {
    let response: BaseResponse<Array<{ id: string; name: string; sub_reasons: any[] }>> =
...
function* confirmDraftOrderSaga(action: YodyAction) {
  ...
  yield put(showLoading());
  try {
    let response: BaseResponse<any> = yield call(confirmDraftOrderService, orderId, params);
    if (isFetchApiSuccessful(response)) {
...
function* orderChangeStoreSaga(action: YodyAction) {
  ...
  try {
    let response: BaseResponse<StoreCustomResponse> = yield call(storesDetailCustomApi, storeId);
    if (isFetchApiSuccessful(response)) {
      setData(response.data);
