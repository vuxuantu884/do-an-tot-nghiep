...
function* listAllOrderSourceSaga(action: YodyAction) {
  ...
  try {
    let response: BaseResponse<PageResponse<OrderSourceResponseModel>> = yield call(
      getSourcesWithParamsService,
      queryParams,
    );
...
export function* settingOrderSourceSaga() {
  ...
  yield takeLatest(SETTING_TYPES.orderSources.listSourceCompany, listAllOrderSourceCompaniesSaga);
