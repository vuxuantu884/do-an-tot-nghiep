...
function* getOrderConfigSaga(action: YodyAction) {
  ...
  try {
    let response: BaseResponse<OrderConfigResponseModel> = yield call(getOrderConfigService);
...
function* getOrderConfigActionSaga(action: YodyAction) {
  ...
  yield put(showLoading());
  try {
    let response: BaseResponse<OrderConfigActionOrderPreviewResponseModel[]> = yield call(
      getOrderConfigActionService,
    );
    if (isFetchApiSuccessful(response)) {
      handleData(response.data);
...
function* editOrderConfigActionSaga(action: YodyAction) {
  ...
  yield put(showLoading());
  try {
    let response: BaseResponse<any> = yield call(editOrderConfigActionService, params);
...
function* getIsAllowToSellWhenNotAvailableStockSaga(action: YodyAction) {
  ...
  yield put(showLoading());
  try {
    let response: BaseResponse<IsAllowToSellWhenNotAvailableStockResponseModel> = yield call(
      getIsAllowToSellWhenNotAvailableStockService,
    );
    ...
    if (isFetchApiSuccessful(response)) {
      ...
      yield put(
        fetchApiErrorAction(response, "Cập nhật cấu hình đơn hàng bán khi tồn kho nhỏ hơn 0"),
      );
    }
  } catch (error) {
    console.log("error", error);
...
function* listConfigurationShippingServiceAndShippingFeeSaga(action: YodyAction) {
  const { handleData } = action.payload;
  yield put(showLoading());
  try {
    let response: BaseResponse<ShippingServiceConfigResponseModel[]> = yield call(
      getListShippingServiceConfigService,
    );
...
function* createConfigurationShippingServiceAndShippingFeeSaga(action: YodyAction) {
  const { params, handleData } = action.payload;
  yield put(showLoading());
  try {
    let response: BaseResponse<any> = yield call(createListShippingServiceConfigService, params);
    if (isFetchApiSuccessful(response)) {
      handleData(response.data);
    } else {
...
function* getConfigurationShippingServiceAndShippingFeeDetailSaga(action: YodyAction) {
  const { id, handleData } = action.payload;
  yield put(showLoading());
  try {
    let response: BaseResponse<ShippingServiceConfigDetailResponseModel> = yield call(
      getShippingServiceConfigDetailService,
      id,
    );
...
function* updateConfigurationShippingServiceAndShippingFeeSaga(action: YodyAction) {
  const { id, params, handleData } = action.payload;
  yield put(showLoading());
  try {
    let response: BaseResponse<any> = yield call(updateShippingServiceConfigService, id, params);
    if (isFetchApiSuccessful(response)) {
...
function* deleteConfigurationShippingServiceAndShippingFeeSaga(action: YodyAction) {
  const { id, handleData } = action.payload;
  yield put(showLoading());
  try {
    let response: BaseResponse<any> = yield call(deleteShippingServiceConfigService, id);
...
export function* settingOrdersSaga() {
  yield takeLatest(SETTING_TYPES.orderSettings.GET_ORDER_CONFIGURATIONS, getOrderConfigSaga);

  yield takeLatest(SETTING_TYPES.orderSettings.GET_ORDER_CONFIG_PRINT, getOrderConfigPrintSaga);

  yield takeLatest(SETTING_TYPES.orderSettings.GET_ORDER_CONFIG_ACTION, getOrderConfigActionSaga);
  ...
  yield takeLatest(
    SETTING_TYPES.orderSettings.CONFIGURE_IS_ALLOW_TO_SELL_WHEN_NOT_AVAILABLE_STOCK,
    configureIsAllowToSellWhenNotAvailableStockSaga,
  );

  yield takeLatest(
    SETTING_TYPES.orderSettings.LIST_CONFIGURATION_SHIPPING_SERVICE_AND_SHIPPING_FEE,
    listConfigurationShippingServiceAndShippingFeeSaga,
  );

  yield takeLatest(
    SETTING_TYPES.orderSettings.CREATE_CONFIGURATION_SHIPPING_SERVICE_AND_SHIPPING_FEE,
    createConfigurationShippingServiceAndShippingFeeSaga,
  );

  yield takeLatest(
    SETTING_TYPES.orderSettings.GET_CONFIGURATION_SHIPPING_SERVICE_AND_SHIPPING_FEE_DETAIL,
    getConfigurationShippingServiceAndShippingFeeDetailSaga,
  );

  yield takeLatest(
    SETTING_TYPES.orderSettings.UPDATE_CONFIGURATION_SHIPPING_SERVICE_AND_SHIPPING_FEE,
    updateConfigurationShippingServiceAndShippingFeeSaga,
  );

  yield takeLatest(
    SETTING_TYPES.orderSettings.DELETE_CONFIGURATION_SHIPPING_SERVICE_AND_SHIPPING_FEE,
